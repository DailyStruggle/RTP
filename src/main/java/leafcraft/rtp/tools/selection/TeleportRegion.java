...
public class TeleportRegion implements TeleportRegionInterface {
...
//        if(locationQueue == null) locationQueue = new ConcurrentLinkedQueue<>();
        ...
        for (int i = 0; i < chunkSet.expectedSize; i++) {
            if(PaperLib.isPaper() || !urgent) {
                ...
                cfChunk.whenCompleteAsync((chunk, throwable) -> {
                    ...
                    //noinspection deprecation
                    Biome biome = RTPAPI.getServerIntVersion()<17
    ...
    public Location getRandomLocation(Set<Biome> biome) {
        ...
        while(numAttempts < maxAttempts && !goodLocation) {
            ...
            switch (state) {
                case SYNC -> {
                    if (SelectionAPI.checkGlobalRegionVerifiers(res)) {
                        chunk = res.getChunk();
                    } else continue;
                }
            ...
            start = System.nanoTime();
            int y;
            if(state.equals(SelectionAPI.SyncState.SYNC)) {
...
//        if(!material.isSolid()) return false;
        ...
        if(!SelectionAPI.checkGlobalRegionVerifiers(block1.getLocation())) return false;
