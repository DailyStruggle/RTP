 ...
 * 128x128 ARGB pixel buffer (NeoForge analogue of {@code FabricMapCanvas}).
 * {@code ChartRenderer}s author against the maps-api logical palette (0..31);
 * this canvas translates each logical byte to an ARGB int via {@link #PALETTE}.
 * The filled buffer is later handed to the active
 * {@code NeoForgeVersionAdapter#renderMapChart} seam, which performs the
 * version-specific ARGB to vanilla {@code MapColor} byte matching and the actual
 * {@code MapItemSavedData} write.
 *
 * <p>Mirrors the {@code BukkitMapCanvas} adapter in
 ...
 * pre-translated vanilla bytes so the per-version carrier keeps ownership of the
 * {@code net.minecraft.*} colour table.
 *
 * <p>No chunk I/O, no blocking (REQ-RTP-MAP-002 / S-005). {@link #commit()} is a
 * no-op: the buffer is read by the carrier after the render pass returns.
 */
public final class NeoForgeMapCanvas implements MapCanvas {
     ...
     */
    public NeoForgeMapCanvas(int[] argb) {
    ...
    public void setPixelRgb(int x, int y, int rgb) {
        ...
        // vanilla MapColor across the ~240-entry palette.
