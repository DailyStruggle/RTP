...
public final class NeoForgeRTPPlayer implements RTPPlayer, NeoForgeBookOpener, NeoForgeMapSink {
    ...
    // The block-data string round-trips through the vanilla BlockStateParser, so it
    // is platform-internally compatible with whatever getClientBlock produced.
    //
    ...
    // section-level binning is not exposed by a public Vanilla constructor for
    // arbitrary fake states, so it stays a Bukkit-only optimisation.
    // ---------------------------------------------------------------------------
    ...
    public String getClientBlock(RTPLocation location) {
        ...
        if (!(location.world() instanceof NeoForgeRTPWorld nw)) return null;
        ...
        if (!nw.isChunkLoaded(x >> 4, z >> 4)) return null;
        try {
            ...
            RTP.log(java.util.logging.Level.FINE,
                    "[RTP][NeoForge] getClientBlock failed at (" + x + "," + y + "," + z + "): " + t);
            return null;
        }
    }
    ...
    public void sendClientBlockChange(RTPLocation location, String blockData) {
        ...
        if (!(location.world() instanceof NeoForgeRTPWorld nw)) return;
        ServerLevel level = nw.level();
        if (level == null) return;
        int x = location.x();
        int y = location.y();
        int z = location.z();
        if (!nw.isChunkLoaded(x >> 4, z >> 4)) return;
        ...
        try {
            ...
            RTP.log(java.util.logging.Level.WARNING,
                    "[RTP][NeoForge] sendClientBlockChange failed for " + name + ": " + t.getMessage());
        }
    }
    ...
    private net.minecraft.world.level.block.state.@Nullable BlockState parseBlockData(
        ...
        try {
            ...
            RTP.log(java.util.logging.Level.WARNING,
                    "[RTP][NeoForge] ignoring malformed client block-data string: " + blockData
