...
/**
 * Shared resolver implementing the formal {@code effective(player)} set
 * (NeoForge analogue of {@code FabricEffectivePermissionsResolver};
 * rtp-fabric-ADR-011):
 *
 * <pre>
 * effective(player) =
 *     DEFAULT_TRUE
 *   ∪ LP_NODES(player.uuid)                              // LuckPerms, if present
 ...
 * </pre>
 *
 * <p>Reused by {@code NeoForgeRTPPlayer} and the console sender on
 * {@code NeoForgeServerAccessor}. Each caller supplies its own UUID, op-flag,
 * and {@code hasPermission} predicate; the resolver does not touch any platform
 * handle directly.
 *
 * <p>{@code EffectFactory.registeredNames()} is reached reflectively so this
 * module does not gain a hard compile-time dependency on {@code effects-api}: if
 * the effects-api jar is absent (lite assembly, test fixtures), the
 * {@code rtp.effect.*} probe simply contributes nothing. The lookup is memoised
 * after first probe.
 */
public final class NeoForgeEffectivePermissionsResolver {
    ...
    private NeoForgeEffectivePermissionsResolver() {}
     ...
     * @param hasPermission predicate routing through the platform's permission
     *                      chain. Must never throw.
    ...
    public static Set<String> resolve(UUID uuid, boolean isOp, Predicate<String> hasPermission) {
        ...
        // 1. DEFAULT_TRUE
        out.addAll(NeoForgeDefaultPermissions.defaultTrue());

        // 2. LP_NODES (LuckPerms primary path)
        out.addAll(LuckPermsNeoForgeEnumerator.grantedNodes(uuid));
        ...
        if (hasPermission != null) {
            for (String event : NeoForgeOnEventPermissions.EVENT_NAMES) {
                String node = NeoForgeOnEventPermissions.PREFIX + event;
                try {
                    if (hasPermission.test(node)) out.add(node);
                } catch (Throwable ignored) {
                    // ignored
        ...
        // LuckPerms) before any on-event teleport fires. Only the rtp.effect.*
        // closed namespace is op-implies-all (its plugin.yml default is `op`).
        if (isOp) {
            for (String id : effectIds) {
                if (id == null || id.isEmpty()) continue;
    ...
    public static Set<String> resolveConsole() {
        Set<String> out = new LinkedHashSet<>();
        out.addAll(NeoForgeDefaultPermissions.defaultTrue());
        out.addAll(NeoForgeOnEventPermissions.OP_GRANTS);
        for (String id : effectIds()) {
            if (id == null || id.isEmpty()) continue;
            out.add((EFFECT_PREFIX + id).toLowerCase(Locale.ROOT));
        }
    ...
    private static boolean effectsApiAvailable() {
        ...
        synchronized (NeoForgeEffectivePermissionsResolver.class) {
