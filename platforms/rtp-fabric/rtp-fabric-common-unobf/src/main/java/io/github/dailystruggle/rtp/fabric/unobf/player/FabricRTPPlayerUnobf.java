...
public final class FabricRTPPlayerUnobf implements RTPPlayer {
    ...
    public void setRespawnLocation(RTPLocation to) {
        ...
        // thread via reflective setRespawnPosition (signature drift across MC
        // versions). S-004: failures are logged, never swallowed.
        ServerPlayer p = handle;
        if (p == null || to == null) return;
        RTPWorld<?> rtpWorld = to.world();
        if (!(rtpWorld instanceof FabricRTPWorldUnobf fw)) return;
        ServerLevel target = fw.level();
    ...
    private static void anchorRespawn(ServerPlayer cur, ServerLevel target, int x, int y, int z) {
        try {
            ...
            RTP.log(java.util.logging.Level.WARNING,
                "[RTP] FabricRTPPlayerUnobf setRespawnPosition failed", t);
        }
    }
