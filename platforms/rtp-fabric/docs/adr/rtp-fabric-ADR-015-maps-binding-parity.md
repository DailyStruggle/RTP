...
- **`rtp-fabric-common`** hosts the platform-neutral, NM-free pieces:
  ...
  - `FabricMapBinding implements MapBinding, MapBindingLifecycle` - owns the per-chart ARGB buffers and the live-refresh loop; resolves the viewer to a `FabricRTPPlayer` through `RTP.serverAccessor` (no NM in its constant pool, no ad-hoc `MinecraftServer` lookup) and delegates all NM work through that player's map sink. `allocate` returns a synthetic `MapHandle.mapId` (the real `MapId` is carrier-owned, keyed by `chartId`); `renderEphemeral`/`bindLive` only paint the buffer; `deliverTo` triggers the player dispatch with `deliverItem=true`; the live loop re-renders and re-dispatches at ~1 Hz with `deliverItem=false`. `onPlayerQuit`/`onDisable` cancel live tasks and release carrier state (REQ-RTP-MAP-003).
  - **The maps path flows player-first.** Server/level state is reached through the viewer's `RTPPlayer`, not a standalone `MinecraftServer`: `FabricRTPPlayer` gains `renderMapChart(chartKey, int[] argb, locked, deliverItem)` (mirroring its `openBookMenu` sink) which resolves the active carrier, hops to the server tick thread via `RTP.scheduler`, and delegates to the carrier seam with its live `ServerPlayer` handle; `releaseMapChart(chartKey)` forwards cache cleanup. The raw `ServerPlayer` never leaves `FabricRTPPlayer`.
- **`FabricVersionAdapter`** gains three NM-free default-method seams: `renderMapChart(Object serverPlayer, chartKey, int[] argb, locked, deliverItem)` (the viewer is supplied as the raw NM `ServerPlayer` the `FabricRTPPlayer` owns), `releaseMapChart(chartKey)` (pure cache cleanup, needs no server or player), and `supportsMapCharts()` (default `false`).
