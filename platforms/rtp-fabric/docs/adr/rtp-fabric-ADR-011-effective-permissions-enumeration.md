...
- **Status:** Accepted (2026-05-22; revised same day to broaden scope beyond `rtp.effect.*` per consumer audit; implemented same day). **Amended 2026-05-31:** the `rtp.onevent.*` family is removed from the op-implies-all expansion in the player path so operators are no longer auto-teleported on events without an explicit grant (see *Amendment* below).
...
- **Related:** [effects-api-ADR-003](../../../../api/effects-api/docs/adr/effects-api-ADR-003-platform-split-bukkit-fabric.md) (platform split that exposed the empty-set stub on Fabric), [effects-api-ADR-005](../../../../api/effects-api/docs/adr/effects-api-ADR-005-effects-yml-config-and-translations.md) (`buildEffects(prefix, Collection<String>)` seam), [effects-api-ADR-006](../../../../api/effects-api/docs/adr/effects-api-ADR-006-fabric-obf-unobf-split.md), `rtp-fabric-ADR-002` (Fabric platform in scope), `rtp-fabric-ADR-007` (Mojmap name decoupling), [ADR-023](../../../../docs/adr/ADR-023-login-reserve-cache.md) (join-time `ParsePermissions.hasPerm("rtp.onevent.", ...)`), [ADR-043](../../../../docs/adr/ADR-043-personal-queue-permission-semantics.md) (`rtp.personalqueue` as op-only direct check, intentionally bypassing `getEffectivePermissions()`), `CHANGELOG.md` entry "On-event teleports granted to operators by default (Paper + LuckPerms)" which is the precedent for routing `rtp.onevent.*` through `getEffectivePermissions()` rather than `hasPermission()`.
...
effective(player) =
  ...
  ∪ (player.isOp ? OP_EFFECT_GRANTS : ∅)            // op-implies-all for rtp.effect.* only (see Amendment 2026-05-31)
```
...
3. **Op-implies-all is restricted to the `rtp.effect.*` closed namespace.** On Bukkit, ops do NOT get `rtp.onevent.*` (those nodes are `default: false`, denied to everyone), and they do not get numeric tails on Fabric either. Documented as divergent behaviour from the original ADR text: per the *Amendment* below, Fabric ops receive only the `rtp.effect.*` op-grant. To receive an on-event teleport an operator must be granted the specific `rtp.onevent.<event>` node explicitly (perms mod / LuckPerms); to override `rtp.delay.<n>` / `rtp.cooldown.<n>` they must install LuckPerms and grant the node, or edit `config.yml`.
...
## Amendment (2026-05-31) - `rtp.onevent.*` excluded from op-implies-all

The original decision expanded `OP_CLOSED_NAMESPACE_GRANTS` to include the entire `rtp.onevent.*` family for op players. In practice this auto-enrolled every operator into on-event teleports: an op was silently random-teleported on join / respawn / world-change without ever opting in. This contradicts `plugin.yml`, which declares each `rtp.onevent.<event>` node as `default: false` (denied to everyone, including ops), and contradicts Bukkit behaviour, where ops do not receive `default: false` nodes.

Decision: the op-implies-all expansion in `FabricEffectivePermissionsResolver.resolve(...)` (player path) now covers the `rtp.effect.*` namespace only. On-event teleports fire for an operator only when the specific `rtp.onevent.<event>` node is granted explicitly (perms mod / LuckPerms), exactly as for a non-op player via `REGISTRY_PROBE`. The `resolveConsole()` path is unchanged (console is genuinely op-equivalent and never triggers an on-event teleport).
