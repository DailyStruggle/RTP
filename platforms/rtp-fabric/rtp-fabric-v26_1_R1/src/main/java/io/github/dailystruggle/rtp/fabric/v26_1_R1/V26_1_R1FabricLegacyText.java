...
final class V26_1_R1FabricLegacyText {
     ...
     * {@code run}, else {@code SUGGEST_COMMAND}). Hover/click are layered across
     * the whole line (Bukkit parity). On any mapping-drift failure the
     * un-decorated component is returned rather than aborting the caller.
     */
    ...
    // ------------------------------------------------------------------
    // Cross-shape HoverEvent / ClickEvent construction (reflective).
    //
    // On MC 26.x HoverEvent / ClickEvent are sealed interfaces whose instances
    // are per-action records (HoverEvent$ShowText(Component),
    // ClickEvent$RunCommand(String), ClickEvent$SuggestCommand(String)); the
    // legacy two-arg constructors are gone. We probe both shapes reflectively
    // (legacy ctor first, then the record carriers) and cache the resolution,
    // mirroring rtp-fabric-common FabricLegacyText so the dispatch is robust
    // across 26.x point releases without a typed binding.
    // ------------------------------------------------------------------
    ...
    /**
     * Read a {@link ClickEvent}'s action enum without naming its accessor,
     * which differs across MC mappings ({@code getAction()} on older releases,
     * {@code action()} on the 26.x record carriers). Returns {@code null} if no
     * known accessor resolves.
     */
