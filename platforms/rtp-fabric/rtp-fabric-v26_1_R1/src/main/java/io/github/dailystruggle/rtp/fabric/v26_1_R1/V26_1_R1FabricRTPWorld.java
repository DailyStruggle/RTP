...
public final class V26_1_R1FabricRTPWorld extends RTPWorld<ServerLevel> {
    ...
    private boolean tryAddTempLoadTicket(ServerLevel level, ChunkPos cp) {
        try {
            ...
            RTP.log(Level.FINE,
                    "[RTP][v26_1_R1] temp load-ticket apply failed for chunk=" + cp + ": "
    ...
    private void scheduleTempLoadTicketRemoval(ServerLevel level, ChunkPos cp) {
        ...
        Runnable remove = () -> {
            try {
                ...
                RTP.log(Level.FINE,
                        "[RTP][v26_1_R1] temp load-ticket release failed for chunk=" + cp + ": "
                                + t.getClass().getSimpleName() + ": " + t.getMessage());
            }
        };
    ...
    private CompletableFuture<ChunkAccess> requestChunkFuture(ServerLevel level, int cx, int cz) {
        try {
            ...
            return ((CompletableFuture<Object>) cf)
                    .thenApplyAsync(V26_1_R1FabricRTPWorld::unwrapChunk, RTP_CONTINUATION_EXECUTOR);
        } catch (Throwable t) {
            RTP.log(Level.WARNING,
    ...
    private static java.lang.reflect.Method resolveGetChunkFutureMethod(ServerChunkCache cache)
        ...
        synchronized (V26_1_R1FabricRTPWorld.class) {
            ...
            // entry point getChunkFuture(...) AND the inner getChunkFutureMainThread(...).
            // We MUST prefer the PUBLIC getChunkFuture, because getChunkAt invokes it
            // OFF the tick thread where only the public entry point takes the
            // non-blocking supplyAsync(mainThreadProcessor) branch. Binding the inner
            // getChunkFutureMainThread off-thread returns a future the tick loop never
            // drives (nullChunk/asyncLoadNull). Fall back to a structural scan for
