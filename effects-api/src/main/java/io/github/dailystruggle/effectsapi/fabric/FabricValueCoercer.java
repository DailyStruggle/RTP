...
public final class FabricValueCoercer implements ValueCoercer {
    ...
    public Object parse(TypeKey type, String raw) {
        ...
        switch (type) {
            ...
            case SOUND: {
                ...
                if (v == null) {
                    FabricEffectRuntime.log(Level.FINER, "[effects-api] [FabricValueCoercer] SOUND miss for token='" + raw
            ...
            case PARTICLE: {
                ...
                if (v == null) {
                    FabricEffectRuntime.log(Level.FINER, "[effects-api] [FabricValueCoercer] PARTICLE miss for token='" + raw
                            + "'; tried keys: [" + triedKeys(raw) + "]");
            ...
            case POTION_EFFECT: {
                ...
                if (v == null) {
                    FabricEffectRuntime.log(Level.FINER, "[effects-api] [FabricValueCoercer] POTION_EFFECT miss for token='" + raw
                            + "'; tried keys: [" + triedKeys(raw) + "]");
    ...
    private static <T> T resolveRegistry(Registry<T> registry, String raw) {
        ...
        ResourceLocation key = tryResourceLocation(raw);
        if (key != null) {
            // Cross-version shim: get(ResourceLocation) was renamed to
            // getValue(ResourceLocation) in MC 1.21.2 (Mojmap). A direct call
        ...
        if (!lower.equals(raw)) {
            ResourceLocation lowerKey = tryResourceLocation(lower);
            if (lowerKey != null) {
        ...
        // (e.g. ENTITY_ENDERMAN_TELEPORT) where the Mojmap ResourceLocation
        ...
        if (dotted != null && !dotted.equals(lower) && !dotted.equals(raw)) {
            ResourceLocation dottedKey = tryResourceLocation(dotted);
            if (dottedKey != null) {
    ...
    private static ResourceLocation tryResourceLocation(String raw) {
        ...
        try {
            ...
            // ResourceLocation API drift across MC versions; fall through.
        }
        try {
            return ResourceLocation.tryParse("minecraft:" + raw.toLowerCase());
        } catch (Throwable ignored) {
            return null;
        }
    }
