 ...
 *
 * <ol>
 *   <li><b>Delay countdown</b> ({@link ExampleKeys#delayCountdown}): driven from {@link
 ...
 *       once per second.
 *   <li><b>Queue countdown</b> ({@link ExampleKeys#queueCountdown}): driven from the core {@link
 ...
 */
public final class ExampleCountdownHooks {
  ...
  private void onSetup(TeleportPipelineTask task) {
    if (!flag(ExampleKeys.delayCountdown)) return;
  ...
  private void onQueuePush(Region region, UUID id) {
    if (!flag(ExampleKeys.queueCountdown)) return;
  ...
  private boolean flag(ExampleKeys key) {
