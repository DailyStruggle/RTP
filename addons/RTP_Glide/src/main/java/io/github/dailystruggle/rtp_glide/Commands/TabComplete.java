  ...
  public void getList(
    ...
    if (i == args.length - 1) { // if last arg
      ...
      if (command.subParams.containsKey(arg)) {
        if (sender.hasPermission(command.subParams.get(arg))) knownParams.add(arg);
      } else if (command.commands.containsKey(
          args[i])) { // if argument is a command, use next layer
        SubCommand subCommand = command.commands.get(args[i]);
        if (sender.hasPermission(subCommand.perm)) command = command.commands.get(args[i]);
      }
