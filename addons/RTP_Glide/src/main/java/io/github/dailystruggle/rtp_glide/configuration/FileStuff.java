...
public class FileStuff {
  ...
  public static void renameFiles(RTP_Glide plugin, String name) {
    ...
    for (int i = 1; i < 1000; i++) {
      File file =
          new File(plugin.getDataFolder().getAbsolutePath() + File.separator + name + ".old" + i);
    ...
    for (int i = toRename.size() - 1; i >= 0; i--) {
      ...
      File newFile =
          new File(plugin.getDataFolder().getAbsolutePath() + File.separator + newFileName);
    ...
    File newFile =
        new File(plugin.getDataFolder().getAbsolutePath() + File.separator + name + ".old1");
    try {
      Files.deleteIfExists(newFile.toPath());
    } catch (IOException e) {
      RTP.log(Level.WARNING, e.getMessage(), e);
