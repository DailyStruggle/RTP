...
public class GroupPlacementEngineTest {
  ...
  @Test
  @DisplayName("GroupShapes resolves square, circle, unknown shapes and null/empty configs")
  void testGroupShapesResolution() {
    assertNull(GroupShapes.resolve(null, 10));
    assertNull(GroupShapes.resolve(Map.of(), 10));
    assertNull(GroupShapes.resolve(Map.of("name", "square"), 10));
    assertNull(GroupShapes.resolve(Map.of("name", "unknown_shape_xyz"), 10));

    Map<String, Object> circleBlock = new HashMap<>();
    circleBlock.put("name", "circle");
    circleBlock.put("radius", 999L);
    circleBlock.put("customParam", 42L);

    io.github.dailystruggle.rtp.common.selection.region.selectors.shapes.Shape<?> resolved =
        GroupShapes.resolve(circleBlock, 15);
    assertNotNull(resolved);
    assertEquals(15L, ((Number) resolved.getParameters().get("radius")).longValue());
  }

  @Test
  @DisplayName("GroupBacklogEntry accessors, validity transitions, and slot mutations")
  void testGroupBacklogEntryLifecycle() {
    RTPCoords anchor = new RTPCoords("world", 100, 64, 200);
    GroupProfile profile = new GroupProfile("duel", Map.of("name", "circle", "radius", 50L), 2);
    List<RTPCoords> candidateSlots = List.of(
        new RTPCoords("world", 101, 64, 201),
        new RTPCoords("world", 105, 64, 205)
    );

    GroupBacklogEntry entry = new GroupBacklogEntry(anchor, 50, profile, candidateSlots);
    assertEquals(anchor, entry.anchor());
    assertEquals(50, entry.blockRadius());
    assertEquals(profile, entry.profile());
    assertEquals(2, entry.candidateSlots().size());
    assertEquals(GroupBacklogEntry.Validity.UNVERIFIED, entry.validity());
    assertTrue(entry.validatedSlots().isEmpty());

    // Null candidates fallback to empty list
    GroupBacklogEntry emptySlotsEntry = new GroupBacklogEntry(anchor, 50, profile, null);
    assertTrue(emptySlotsEntry.candidateSlots().isEmpty());

    // Validity updates
    entry.setValidity(GroupBacklogEntry.Validity.VALIDATED);
    assertEquals(GroupBacklogEntry.Validity.VALIDATED, entry.validity());
    assertTrue(entry.compareAndSetValidity(GroupBacklogEntry.Validity.VALIDATED, GroupBacklogEntry.Validity.INVALIDATED));
    assertEquals(GroupBacklogEntry.Validity.INVALIDATED, entry.validity());
    assertFalse(entry.compareAndSetValidity(GroupBacklogEntry.Validity.VALIDATED, GroupBacklogEntry.Validity.UNVERIFIED));

    // Validated slots mutation
    RTPLocation loc1 = new RTPLocation(candidateSlots.get(0), 1);
    RTPLocation loc2 = new RTPLocation(candidateSlots.get(1), 1);
    entry.addValidatedSlot(loc1);
    entry.addValidatedSlot(null);
    assertEquals(1, entry.validatedSlots().size());

    entry.setValidatedSlots(List.of(loc1, loc2));
    assertEquals(2, entry.validatedSlots().size());

    entry.setValidatedSlots(null);
    assertTrue(entry.validatedSlots().isEmpty());
  }
}
