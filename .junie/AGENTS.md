...
| "backlog cache" / "L3" / "L3 cache" / "binned cache" | Proposed ([ADR-028](../docs/adr/ADR-028-l3-backlog-cache.md)) — `RegionQueueManager.backlogLocations` (new `BacklogLocationBuffer`); not yet a code symbol | Optional **unverified** buffer upstream of `unkeptLocations`. Order-preserving FIFO with a per-entry `verified` flag, head-blocking promotion, anvil-prefilter verification one bin (32×32 chunks = one `.mca`) per `Region.execute()` pulse. Cap key: `backlogCacheCap` (default 1000; lite default 0). Not persisted to the DB. |
