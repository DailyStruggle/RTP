...
4. **Documentation website (docs-as-code, MkDocs Material)** -- goal: publish a generated site from the repo's Markdown, in the docs-as-code style. **In progress (2026-08-15).** Done: root `mkdocs.yml` (Material theme, `docs_dir: docs`, admonition/content-tab extensions, published `nav` over `site/` + `admin/` + developer pages, `not_in_nav` excluding the internal `dev/`/`adr/`/`architecture/` trees), pinned `docs/requirements.txt`, and a green local `mkdocs build`. Narrative source lives under `docs/site/`; the generated `/site/` output is gitignored. Open: auto-publish CI (a `.github/workflows/docs.yml` running `mkdocs gh-deploy --force`, or a `Jenkinsfile` build stage); consolidating the detached `wiki/` reference pages into `docs/` (D-005-adjacent, awaiting scope confirmation) plus converting absolute wiki links to relative paths; enabling GitHub Pages hosting; and optional polish (`mike` versioning, custom domain/logo). Working checklist: [`docs/dev/scratch/CHECKLIST-docs-site.md`](../docs/dev/scratch/CHECKLIST-docs-site.md).
