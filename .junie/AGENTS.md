...
* **REQ-RTP-S-005** — The system shall not perform chunk loading or validation on the main server thread. All such operations shall be dispatched through the platform-appropriate async scheduler.
  - *Already satisfied by:* `LocationGenerator.getLocation(Region, Set<String>)` calls `world.getChunkAtAsync(cx, cz)` (returns a `CompletableFuture`) and the entire method is invoked from `TeleportPipelineTask.runSetup()`, which runs under `AsyncTaskProcessing`. Platform adapters (`rtp-paper`, `rtp-folia`) override `getChunkAtAsync` with their native async APIs. Do **not** call synchronous `world.getChunkAt()` directly from any new code path that may execute on the main thread.
