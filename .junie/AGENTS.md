...
| "the proxy plan" / "multi-server plan" / "network mode" | [`docs/dev/MULTI_SERVER_PLAN.md`](../docs/dev/MULTI_SERVER_PLAN.md); canonical terms in [`GLOSSARY.md`](../docs/dev/GLOSSARY.md) (*Backend*, *Proxy*, *Transport*, *Network Snapshot*, *Backend Selector*, *Reservation Token*) | Velocity/BungeeCord cross-server work, D-005 gated. Distinct from `MULTI_PLATFORM_PLAN.md`. Phase 0 partially complete: REQ-RTP-NET-001…014 authored, GLOSSARY entries added; ADR-025 still outstanding. |
