...
- **REQ-RTP-F-001 — Instant Execution:** The system must provide response times at 0-2 gameticks (0-100ms) on average upon command execution, or postpone teleportation to maintain the backend rhythm.
- **REQ-RTP-F-002 — Configurable Geometry:** The system must support various spatial boundaries, including but not limited to circles, squares, and rectangles.
- **REQ-RTP-F-003 — Statistical Distributions:** The system must allow server administrators to configure the mathematical distribution of teleport locations (e.g., Flat, Normal, Exponential).
- **REQ-RTP-F-004 — Region Management:** The system must allow the server to be divided into multiple teleport regions, each with independent configurations, rules, and permissions.

### 1.2 Computational Safety and Performance
- **REQ-RTP-F-005 — Bounded Selection Complexity:** Location selection tasks must operate within deterministic time complexity bounds (e.g., O(log(n))) to ensure predictable computational overhead during background generation.
- **REQ-RTP-F-006 — Strict Execution Time Guarantees:** Location generation algorithms must execute within strict deterministic bounds. The system shall not use naive "rerolling" upon hitting invalid regions.
- **REQ-RTP-F-007 — Algorithmic Uniformity:** The system must ensure uniform spatial distribution and preemptively subtract known invalid sectors.

### 1.3 Resource Management
- **REQ-RTP-F-008 — Non-Blocking Execution:** Location discovery and chunk validation must not block the main server thread.
- **REQ-RTP-F-009 — Redundant Calculation Elimination:** The system must eliminate redundant calculations during future teleport selections.

### 1.4 Integrations and Extensibility
- **REQ-RTP-F-010 — API Access:** The system must expose a robust, decoupled API (`rtp-api`) allowing external plugins to register custom shapes, vertical adjustors, and validation checks.
- **REQ-RTP-F-011 — Claim and Protection Checks:** The system must support integrations with third-party land protection plugins (e.g., GriefPrevention, WorldGuard) to prevent players from teleporting into claimed or restricted areas.

## 2. Non-Functional Requirements

### 2.1 Fault Tolerance
- **REQ-RTP-NF-001 — State Persistence:** The system must maintain algorithmic efficiency across server restarts without rebuilding state from scratch.

### 2.2 Platform Compatibility
- **REQ-RTP-NF-002 — Cross-Platform Thread Safety:** The system must adapt its concurrency model to the underlying server platform (Spigot, Paper, Folia), ensuring strict thread safety and adherence to platform-specific asynchronous APIs or region-based multithreading constraints.

## 3. Prohibition Requirements

These requirements define what the system **must never do**. Each maps to one or more hazards in
...
- **REQ-RTP-S-002 — No Persistent Force-Loaded Chunks:** The system shall not leave a chunk in a
  force-loaded state beyond the configured reservation window. Every chunk ticket acquired must
  be released either by explicit close, by `MemoryTracker` watchdog, or by JVM weak-reference
  collection.
...
- **REQ-RTP-S-004 — No Silent Failure:** The system shall not silently discard a teleport
  request. Every failure (empty queue, invalid region, permission denied, safety rejection) must
...
- **REQ-RTP-S-005 — No Synchronous Chunk I/O on the Main Thread:** The system shall not perform
  chunk loading or validation on the main server thread. All such operations must be dispatched
...
- **REQ-RTP-S-006 — No Undefined Behaviour on Early API Access:** The system shall not produce
  ...
  has finished loading. An `IllegalStateException` with a descriptive message must be thrown
  instead.

## 4. System Requirements
- **REQ-RTP-SYS-001 — Runtime Environment:** Java 21 or higher.
- **REQ-RTP-SYS-002 — Server Software:** Compatible with Bukkit-derived server software, specifically supporting Spigot, Paper, and Folia implementations.
