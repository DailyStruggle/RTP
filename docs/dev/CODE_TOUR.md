...
3. **`LocaleOverlay.apply`** ([ADR-020](../adr/ADR-020-localeoverlay-for-messagesyml.md) / REQ-RTP-F-013). Called on the fresh `messages.yml` parser before it is inserted into the map. No-op for `"en"` or when the locale file is missing. *Break:* "messages still in English despite `language: de`" → the overlay file is missing, or the `language` key value has extra whitespace (the code trims, but only once).
...
> Deep read (optional): [`DESIGN.md`](DESIGN.md) for the per-world / per-region override resolution, [ADR-020](../adr/ADR-020-localeoverlay-for-messagesyml.md) for the locale overlay, [`LESSONS_LEARNED.md`](LESSONS_LEARNED.md) for prior reload / database-flush pitfalls.
...
9. **`ScanTask.kill`, `redisManager.shutdown`, `serverAccessor.stop`.** Static registry clears and platform hooks. On Folia, `serverAccessor.stop` must run on the global region scheduler — the accessor handles that internally; don't relocate the call.
