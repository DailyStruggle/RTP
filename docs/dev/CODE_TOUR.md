...
| Symptom | Section |
|---|---|
| "`/rtp` takes seconds / blocks the server" | [section 2 Teleport pipeline](#2-teleport-pipeline-end-to-end) |
| "Queue never refills / always empty" | [section 3 Budgeted cache generator](#3-budgeted-cache-generator-queue-refill) |
| "Chunks stay loaded forever / RAM grows" | [section 4 Chunk ticket lifecycle](#4-chunk-ticket-lifecycle) and [section 5 Active GC sweep](#5-active-gc-sweep) |
| "`/rtp scan` stalls, crashes, or never finishes" | [section 6 Scan task crawler](#6-scan-task-crawler) |
| "I don't know which module to edit" | [section 1 Module map](#1-module-map-where-does-my-change-go) |
| "Teleport silently does nothing / no error message" | [section 7 Failure attribution and user feedback](#7-failure-attribution-and-user-feedback) |
| "Folia throws `ThreadAccessException` / wrong region" | [section 8 Folia threading gotchas](#8-folia-threading-gotchas) |
| "Plugin doesn't start / missed WorldLoadEvent / integrations didn't hook" | [section 10 Plugin setup lifecycle](#10-plugin-setup-lifecycle) |
| "Why did `/rtp` pick *that* world/region? / override loop error" | [section 11 How behaviors are decided](#11-how-behaviors-are-decided) |
| "Why was this spot rejected? / too many `vert` / `biome` / `safety` misses" | [section 12 Location selection per attempt](#12-location-selection-per-attempt) |
| "Changed a config value and it didn't take effect / `/rtp reload` leaks / messages stuck in English" | [section 13 Configuration load and reload](#13-configuration-load-and-reload) |
| "Server stop hangs / cached locations lost after restart / leaked chunks on `/stop`" | [section 14 Shutdown and flush lifecycle](#14-shutdown-and-flush-lifecycle) |
...
1. **`CmdTrigger` → `QueryCache`.** The player runs `/rtp`. The command lives in `commands-api` and the Bukkit dispatch in `rtp-plugin/.../bukkit/commands/`. *Break:* if the user sees nothing at all, the command probably failed parameter validation — see REQ-RTP-S-007 ("busy" / "invalid command" messages must be configurable via `messages.yml`) and section 7 below.

2. **`QueryCache` choice node.** `RegionQueueManager` checks whether a pre-validated location is already waiting. *Break:* if teleport is instant when the server is idle but slow under load, the region's queue is starving — jump to [section 3](#3-budgeted-cache-generator-queue-refill).
...
5. **`ReqTicket` — LOAD stage.** Here the platform adapter is called to asynchronously acquire a chunk ticket. This is the single highest-risk step in the whole plugin:
   ...
   - On pure Spigot, `BukkitRTPWorld.loadChunkFuture` may bounce to tick; see [`DESIGN.md section rtp-bukkit Implementation Notes`](DESIGN.md) and [ADR-016](../adr/ADR-016-anvil-subsystem.md) for the Anvil pre-filter that avoids a load.
...
7. **`EvalBlocks → GenRandom` retry / `Teardown` max-retries.** Bounded by configuration (`maxAttempts`). *Break:* silent exhaustion → see section 7, and the regression guard `ReqRtpS004NullChunkAttributionTest` mentioned in [`.junie/AGENTS.md`](../../.junie/AGENTS.md).
...
9. **`Teardown` — Cleanup phase.** `TeleportPipelineTask.runCleanup()` releases the chunk reservation, untracks from `MemoryTracker`, and decrements `inFlightCalculations`. This **must** run on every exit path — normal, exception, disconnect, cancel. Jump to [section 4](#4-chunk-ticket-lifecycle).

> Deep read (optional): [`DESIGN.md section Pipeline Phases`](DESIGN.md), [`REQUIREMENTS.md section 3`](REQUIREMENTS.md) for all S-00x.
...
> Deep read (optional): [`DESIGN.md section Pulse-Driven Maintenance`](DESIGN.md).
...
2. **Stalled pipeline:** `SweepTask → ForceClose` from the background GC (see [section 5](#5-active-gc-sweep)).
...
> Deep read (optional): [`DESIGN.md section Chunk Allocation Management`](DESIGN.md), [`.junie/AGENTS.md` Code & Testing Conventions](../../.junie/AGENTS.md).
...
- **Scan looks like it leaks.** It shouldn't — it goes through the same ticket lifecycle from [section 4](#4-chunk-ticket-lifecycle). If it does, look at the `DrainGate` path: releases happen in the callback, so an exception in `VertAdjust` / `PhysBiome` *before* `ReleaseGate` leaks a permit and a ticket.
...
> Deep read (optional): [`TRACEABILITY.md`](TRACEABILITY.md) row `REQ-RTP-F-013`, [`REQUIREMENTS.md section 3`](REQUIREMENTS.md) for S-004/S-007.
...
> Deep read (optional): [`DESIGN.md section rtp-folia Implementation Notes`](DESIGN.md), [`.junie/AGENTS.md` Folia Threading](../../.junie/AGENTS.md).
...
5. Edit the correct module per [section 1](#1-module-map-where-does-my-change-go). Before editing any *uncommitted code* file, make a `.bak` copy ([`.junie/AGENTS.md` Backup Policy](../../.junie/AGENTS.md)).
...
1. **Command layer.** Parameter validation failures route to `msgBadParameter`; a server already saturated with in-flight calculations routes to `msgBusy`. Both strings are configurable (S-007, REQ-RTP-F-013). If a user says "nothing happens", this is the first lens — see also [section 7](#7-failure-attribution-and-user-feedback).
...
- **"Shape feels clustered."** Not a decision-tree bug — see [section 2 step 4](#2-teleport-pipeline-end-to-end) and [ADR-001](../adr/ADR-001-archimedean-spiral-1d-mapping.md).
...
> Deep read (optional): [`REQUIREMENTS.md section 3`](REQUIREMENTS.md) (S-007 configurable messages), [`docs/admin/EVENTS_AND_EFFECTS.md`](../admin/configuration/EVENTS_AND_EFFECTS.md), [`GLOSSARY.md`](GLOSSARY.md) for the canonical meaning of *region* vs *world* vs *shape*.
...
This is the **decision core** of the plugin. Every caller you met earlier — the `/rtp` command ([section 11](#11-how-behaviors-are-decided)), the cache generator ([section 3](#3-budgeted-cache-generator-queue-refill)), `/rtp scan` ([section 6](#6-scan-task-crawler)) — ends up asking `ILocationGenerator` for a coordinate, and that coordinate comes out of this loop. If it emits a bad `(x, y, z)` or emits one too slowly, *everything downstream looks broken*. That is why it gets its own diagram separate from the outer attempt-loop plumbing.
...
6. **`BuildRegions` → Dormant decision.** For each `regions/*.yml`, `RegionConfigLoader.load` produces a `RegionSettings`, then `detectFallbackConfiguredWorld` checks whether the configured world is already loaded. Yes → live region. No → **dormant region** (world is `null`, rebinds on `WorldLoadEvent` via `OnWorldLoadUnload.rebindWorld`). This is the Multiverse-compatibility path — see also [section 10 step 3](#10-plugin-setup-lifecycle). *Break:* "region never activates when my world loads" → confirm the world name matches, and that `OnWorldLoadUnload` is registered (diagram 07).
...
Shutdown is the symmetric partner of [section 10 Plugin setup lifecycle](#10-plugin-setup-lifecycle). Unlike a reload (section 13), which reuses allocations, a shutdown must *release* every allocation made during `onEnable`. Two classes of bug dominate this path: **data loss** (cached locations not flushed before the DB stop flag is set) and **resource leaks** (chunk tickets not released — an S-002 violation).
...
4. **Database flush sequence — load-bearing ordering.** `SQL flush → rebuildCachedLocationsFromMemory → flushDirtyCache → processQueries(MAX)`, **all before** `stop.set(true)`. Regression test: `MemoryShapeShutdownTest`. *Break:* "cached locations gone after restart" is almost always a reorder here. `processQueries` bails immediately if `stop` is already set, so setting the flag early silently drops the drain. See [`LESSONS_LEARNED.md`](LESSONS_LEARNED.md) section "Shutdown ordering".
...
> Deep read (optional): [`LESSONS_LEARNED.md`](LESSONS_LEARNED.md) section "Shutdown ordering", [`TRACEABILITY.md`](TRACEABILITY.md) row `REQ-CORE-NF-001` (deterministic shutdown persistence), [`REQUIREMENTS.md section 3`](REQUIREMENTS.md) S-002 (no permanently force-loaded chunks).
