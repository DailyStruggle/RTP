...
>
> **Current scope reminder (updated 2026-06-02):** NeoForge is **in scope** per [ADR-033](../adr/ADR-033-neoforge-platform-in-scope.md) (Accepted), the Fabric stabilization gate is **clear** (Fabric confirmed stable 2026-06-01), the D-005 proposal ([`scratch/PROPOSAL-neoforge-bringup.md`](scratch/PROPOSAL-neoforge-bringup.md)) is **approved**, and the named maintainer is assigned (project lead, `@leaf_26`). The **Phase N1 module skeleton has landed** under [`platforms/rtp-neoforge/`](../../platforms/rtp-neoforge/README.md): ModDevGradle build wiring, `neoforge.mods.toml`, the `@Mod` entry point, a complete server-thread scheduler, and command/version-adapter scaffolds, included only via the opt-in `-PincludeNeoforge` flag (the default offline build is untouched). **Still open:** the `RTPServerAccessor` implementation, command Brigadier-bridge wiring, the S-005 async chunk path, the `/rtp` round-trip exit gate, the S-005/S-006 REQ-traceable guards, and a network-host build verification.
