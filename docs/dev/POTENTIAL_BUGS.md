...
### 2026-09-13 - `DynamicWorldConfigTest` and `ReqRtpNet015NetworkWaitlistTest` failures in `rtp-core:test`

- **Severity:** Medium
- **Status:** Open
- **Discovered during:** verification of unit tests for usage limits placeholders.
- **Location:** `rtp-core/src/test/java/io/github/dailystruggle/rtp/common/configuration/DynamicWorldConfigTest.java` and `rtp-core/src/test/java/io/github/dailystruggle/rtp/common/network/ReqRtpNet015NetworkWaitlistTest.java`.
- **Symptom / hypothesis:** `DynamicWorldConfigTest` fails 3 tests (`testLoadExistingConfigs`, `testConfigReloading`, `testDynamicConfigCreation`) and `ReqRtpNet015NetworkWaitlistTest` fails 2 tests during `./gradlew build`.
- **Impact:** Multi-module `./gradlew build` fails in `:rtp-core:test`; isolated `PlaceholderProviderTest` succeeds.
- **Suggested next step:** Review mock/temp directory lifecycle in `DynamicWorldConfigTest` and network waitlist mock conditions in `ReqRtpNet015NetworkWaitlistTest`.
