...
| 2 | `AnvilChunkView` + palette semantics | `AnvilChunkView` / `PaletteSection` immutable records, block-state index decoder for packed-palette long arrays, per-section block-at-(x,y,z) accessor used by the verdict layer. Still not wired in. | `:rtp-spigot:rtp-spigot-common:test` green. |
