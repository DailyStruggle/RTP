...
### A `MemoryShape` addresses the annulus `[centerRadius, radius)`, so a harness must pin `centerRadius` (2026-09-05)

`Square.xzToLocation` returns `((R*R - cr*cr) * 4) + perimeterStep` for ring `R = max(|x-cx|, |z-cz|)`. Every ring inside `centerRadius` therefore maps to a **negative** 1D index, which `MemoryShape.addBadLocation` refuses (counted by `getOutOfDomainMarkCount()`, logged at `FINE` on the first occurrence and each power of ten). That refusal is by design - the region never selects inside its centre hole - but it silently invalidates any measurement that marks an origin-centred footprint without setting `centerRadius`.

Signature: retention rises with radius while `spatialResolution` is `1`. `ModelCrossoverBenchmarkTest` set only `radius` and inherited the default `centerRadius` of `64`, giving spiral retention of `0.000` at cell radius 32/64, `0.001` at 128, `0.240` at 256 and `0.929` at 512 - exactly the fraction of the footprint lying outside ring 64. It reads as a structural defect in `Square` and is not one: with `centerRadius` pinned to `0` the shape retains every offered in-domain mark exactly (`SquareRetentionTest`), so `xzToLocation` is injective over a full cell square and the flush/rebuild path loses nothing.

Two rules for any future harness or scan that marks cells directly:

- Set `centerRadius` explicitly to match the domain being marked, and iterate `-radius+1 .. radius-1` so the outer ring stays below `getRange()`.
- Assert `getOutOfDomainMarkCount() == 0` after the fill rather than reporting retention as a result. A footprint or byte figure for a structure that dropped its input is worse than no figure.

Note that `setSpatialResolution` clamps to `>= 1` and the coalescing test is `nextKey <= start + length + spatialResolution`, so a one-cell gap is bridged: `getEffectiveBadCount()` is run-length-derived and can *exceed* the number of marks offered on a sparse pattern. It can never fall below it.
...
Caveats: this removes the arrival-ring confound but **not** the intra-plugin `ScanTask` background-fill confound from the section 5h `‡` footnote (scan loads are far from the destination, so they survive the distance gate and still inflate the raw attributed count); for a pure pipeline-cost number, disable RTP's queue/scan as in the section 5e queues-off configuration. Cross-plugin chunk-cost comparisons should now quote `chunks_selection_per_attempt`. New columns are appended at the end of both CSVs so existing parsers (`spark_summary.py`, downstream scripts) stay compatible.
...
The section 5e run kept the same two clients and the same four plugins as sections 5b-5d, but changed three things at once:
...
What section 5e shows at a glance:
...
- **Cross-phase chunk residency is not accumulating.** Spark's loaded-chunk count is flat at ~1600–1800 across all four 5-min phases. The sections 5b-5d 60 s windows ended before this could be observed; once a phase is long enough to reach steady state, the loaded-chunk count just stabilises.
...
- **`rtp` main-thread CPU/attempt is reproducible across four independent runs**: section 5b 572 ms, section 5c slot 1 518 ms, section 5d slot 2 542 ms, section 5e slot 4 565 ms. ±5 % spread across queue-on/off and 60 s/300 s phases — strong publishable headline.
- **chunks/att is now meaningful**: 79 / 70 / 76 / 77 across the four plugins under attributed measurement, with the per-phase background bucket ≤ 1 % of attributed. Pre-section 5e numbers (the global counter, with queues on) were upper bounds inflated by inter-player crosstalk and pre-warm — do not compare sections 5b/5c/section 5d chunks/att head-to-head against section 5e or against new runs.
- **Cold-start cost re-emerges with queues off.** Queue-on runs (sections 5b-5d) reported RTP cold-starts of 14–52 ms; section 5e shows 1037 ms. The queue-on number was queue-fetch latency, not teleport pipeline latency. The section 5e cold-start is the actual pipeline cost of the first dispatch.
...
- **Use 5-min phases minimum.** 60 s phases never reach steady state on Spigot 1.20.1 under this workload, so any TPS / latency-tail claim from a 60 s run is a transient. sections 5b-5d numbers on those axes should be treated as superseded by section 5e.
- **Compare plugins on delivered TP/s, main-thread CPU/attempt, and p99-success latency** — not on end-of-phase server `last1m`. Under saturating offered load, server TPS collapses to a platform-side tick-saturation floor common to every plugin and stops discriminating. TP/s, CPU/att, and p99 all rank the plugins consistently in the section 5e data and are the cross-checked axes for the public write-up.
- **Spigot main-thread temporal attribution is exact only at concurrency = 1.** section 5e ran 2 clients with `per-player-gap-ticks: 3` (concurrency 1–2) and produced ≤ 1 % background residual — small but non-zero. Reduce to concurrency = 1 if exactness matters; accept the residual otherwise.
...
- **A `per-player-gap-ticks: 0` Paper run remains pending** to establish RTP's saturation throughput on Paper. From section 5h's 105 ms p50 the predicted ceiling is in the 15–20 TP/s band, but predicted only.
...
Comparison vs Spigot section 5g, same plugins/configs/world:
...
- **BetterRTP "queue presence ≠ queue effectiveness" reconfirmed on Paper.** Same finding as Spigot section 5g, now cross-platform: `Queue.Enabled: true` on Paper still produces p99 = 852 ms vs RTP's L1-served 154 ms. BetterRTP's single-location-per-player queue cannot decouple latency from chunk-gen the way RTP's L1 pool can, on either platform.
...
Run `20260502-171548` (Paper 1.20.1, four plugins, queues on, OPed clients, 2-min phases, 120 s gap, `per-player-gap-ticks: 0`) re-ran the section 5h matrix with **two RTP knobs raised**: `regions/default.yml cacheCap: 10 → 100` (10× deeper L1 pool) **and** `performance.yml period: 10 → 1` (background scan/refill task fires every tick instead of every 10). All other plugin configs unchanged. The intent: cache-size-comparable to JakesRTP's `cache-locations: 10` plus its own backlog, refilled fast enough to never drain mid-phase.

| Plugin     | TP/s section 5h (`cacheCap 10`) | TP/s section 5i | p99 section 5h (ms) | p99 section 5i (ms) | Main CPU/att section 5h | Main CPU/att section 5i |
|------------|-------------------------:|---------:|-------------:|-------------:|-----------------:|-----------------:|
...
- **Knob change isolates correctly: only RTP's row moves.** BetterRTP / HuskHomes / JakesRTP TP/s, p99, and main-CPU/att are within run-to-run variance vs section 5h despite the RTP cache changes — the harness measures plugin-side configuration changes cleanly when they touch only one plugin's config tree.
...
- **The publishable RTP recommendation for sustained-load servers is `cacheCap: 100, period: 1` on Paper 1.20.1**, capable of serving p99 = 4 ms at the harness's 2-client offered ceiling. Smaller caches or longer refill periods will trade tail latency for background CPU; the section 5h `cacheCap: 10, period: 10` regime is the reasonable default for low-traffic servers.
- **A reverse-order rerun at `cacheCap: 100, period: 1` would settle remaining slot-1/slot-4 confounds** (RTP's 0 ms cold and JakesRTP's 21 ms cold). The current section 5i is n=1 in the original phase order.
- **Higher concurrency is now required to find RTP's saturation ceiling.** Both rtp and jakesrtp are pinned at the 2-client × 20-tick = 20 TP/s offered ceiling; the actual plugin ceilings are unknown. A bot-harness ADR (cf. section 6b) is the only way past this rig's ceiling.

---

## 2026-05-02 - section 5i headlines reproduce at n=2 on a same-config consecutive run (5j)

Run `20260502-174114` (Paper 1.20.1, four plugins, queues on, OPed clients, 2-min phases, 120 s gap, `per-player-gap-ticks: 0`, RTP `cacheCap: 100` + `period: 1`) is a deliberate same-order, same-config rerun of section 5i to test n=2 reproducibility before publishing.

| Plugin     | TP/s section 5i | TP/s section 5j | p99 section 5i (ms) | p99 section 5j (ms) | Main CPU/att section 5i | Main CPU/att section 5j |
|------------|---------:|---------:|-------------:|-------------:|-----------------:|-----------------:|
...
- **The section 5i p99 collapse is real, not a single-run artefact.** RTP's p99 reproduces at 3–4 ms across both runs; n=2 is enough to publish the `cacheCap: 100` + `period: 1` recommendation without an "n=1" footnote. The remaining 1 ms variance is below spark sampling resolution.
...
- **Main-CPU/att moved 10–27 % lower across the board on the second run** (jakesrtp -27 %, betterrtp -20 %, rtp -13 %, huskhomes -10 %). The ranking is preserved (rtp < jakesrtp < huskhomes < betterrtp on both runs) but the absolute number swings rig-side, not plugin-side. Two consistent explanations: extended JIT warm-up over the 11-min interval before section 5j and Paper's chunk caches reusing more pre-loaded state. **Do not quote main-CPU/att to two significant figures from a single 2-min phase; quote a range or rerun for the published number.**

Methodological consequences:

- **For publication: TP/s and p99 reproduce well enough to quote n=2 averages from section 5i + section 5j directly.** Main-CPU/att should be quoted as a range across both runs rather than a single number.
- **The reverse-order rerun flagged in section 5i.4 is *still* outstanding** — both section 5i and section 5j ran the same `rtp → betterrtp → huskhomes → jakesrtp` order, so any slot-1-vs-slot-4 carry-over is common-mode in this n=2 verification and would not be detected. A `jakesrtp → huskhomes → betterrtp → rtp` rerun is the cleanest way to settle whether RTP's slot-1 0–1 ms cold-start is order-dependent.
...
Run `20260502-181051` (Paper 1.20.1, three additional RTP-style plugins targeted: `asyrtp:rtp`, `ezrtp:rtp`, `sorekillrtp:rtp`; same harness config as sections 5i/5j: queues on where applicable, OPed clients, `dispatch-as-player: true`, `per-player-gap-ticks: 0`, 2-min phases, 120 s gap). Two of three targets did **not** measure their intended plugin.
...
- **AsyRTP delivers 2.04 TP/s with full success but a ~2 s p99 tail under saturating offered load.** Spark MSPT_max for the asyrtp window is **1343–1768 ms** (single-tick stalls > 1 second), median 3.3–3.7 ms — the long ticks are concentrated in a handful of attempts rather than smeared. AsyRTP's main-CPU/att (43.18 ms) is in the same band as BetterRTP section 5j (43.01 ms) and HuskHomes section 5j (47.19 ms), but its delivered TP/s (2.04) is roughly 1/3 of those plugins'. The bottleneck is not per-attempt main-thread CPU; it is something gating attempt-to-attempt pacing (worker-thread contention, internal serialisation, retry budget, or async-completion plumbing). **Publishable as the third pipeline-served plugin in the four-plugin Paper section 5h matrix**, with the caveat that its tail is dominated by occasional multi-second main-thread stalls rather than steady per-attempt cost.
...
## 2026-05-02 - Post-workaround Paper 1.20.1 four-plugin extension: EzRTP / AdvancedRTP / EssentialsX measured for the first time, AsyRTP section 5k reproduced (5L)

Run `20260502-185238` (Paper 1.20.1, four targets `asyrtp:rtp world world {player}`, `ezrtp:forcertp {player} world`, `advancedrtp:rtp {player}`, `essentialsx:tpr {player}`; queues on where applicable, OPed clients, `dispatch-as-player: true`, `per-player-gap-ticks: 0`, 2-min phases, 120 s gap). After the section 5k cooldown / countdown / target-form config edits, three of the four targets that were previously unmeasured produced their first publishable rows; AsyRTP reproduced section 5k as an n=2 control.
...
- **The `chunks_per_attempt_attributed` axis discriminates own-plugin from sections 5g/5k command-map collisions.** Under a sections 5g/5k collision the harness measures our RTP queue-served path (chunks/att ≈ 0.3, main-CPU/att ≈ 17 ms — see section 5j RTP row). EzRTP's 99.2 chunks/att, AdvancedRTP's 47.7, and AsyRTP's 12.3 are all ≥ 12× the collision signature; EssentialsX's 2.78 is low but its 88.9 ms main-CPU/att rules out collision (which would give low CPU/att too). **All four section 5L rows are the targeted plugins, not our RTP plugin under a label.** This is now the standard post-run check for any new third-party RTP target before publishing its row.
...
- **Cross-plugin chunks/att now spans ~150×** across queued (RTP section 5j ≈0.3 attributed) and pipeline-served (EzRTP section 5L 99.2) extremes in published runs. The attributed-chunk counter is producing a meaningful per-plugin discriminator at saturating offered load, modulo the RTP scan-task footnote (‡) on RTP itself.
- **All section 5L rows are n=1 except AsyRTP (n=2 with section 5k).** Per the section 5j precedent (10–27 % main-CPU/att variance run-to-run), two-significant-figure precision is unjustified for the EzRTP / AdvancedRTP / EssentialsX rows. Publish ranges or rerun before quoting headline numbers.
...
- **HuskHomes is the most striking Folia win.** Spigot section 5g p99 = 5 124 ms → Folia p99 = 901 ms (5.7× improvement). The no-queue Spigot/Paper failure mode disappears almost entirely on Folia because the region thread can absorb the per-attempt chunk-gen cost without dragging the rest of the server. Curiously its Paper section 5h p99 (313–372 ms) is *better* than Folia — likely because Paper's chunk-gen pipeline is mature and per-attempt while Folia's region threads share chunk-gen with player ticking. Worth a footnote in any Folia-vs-Paper claim.
...
- **Mid-run reads of `phases.csv` are dangerous.** An earlier draft of this entry (and section 5M) published BetterRTP at 1.13 TP/s based on a partial mid-run snapshot — the harness flushes per-attempt rows continuously but `phases.csv` only finalises at phase end. **Always wait for the run-final `phases.csv` row count to match the configured phase count before publishing any per-plugin number.**
- **n=1 on Folia for all three plugins.** Quote ranges (or rerun) before any two-significant-figure publication; the section 5j ±15–25 % main-CPU/att band applies until proven otherwise on Folia.
...
## 2026-09-05 - `MemoryShape` biome table rework: the traps that actually cost time

The biome table moved from per-biome `long[]` run tables to a single blocked union (`.bin` version 4). The design and its measurements live in [ADR-081](../adr/ADR-081-unified-blocked-biome-run-table.md); recorded here are only the failures that were silent, misleading, or cost a debugging loop.

- **A second publication path must re-check every field the first one was the sole writer of.** Only a rebuild ever set `totalBiomeCount`, so publishing the union at load left `getEffectiveGoodCount()` at zero. No exception, no warning - just a silently wrong good/bad ratio in the selector.
- **Sizing block-indexed arrays from `count / BLOCK_SIZE` is wrong.** A block closes early whenever the next key or sum offset would exceed `Integer.MAX_VALUE`, so a table can hold more blocks than its run count implies - worst case one per run. Immediate `ArrayIndexOutOfBoundsException` on the first table with a gap wider than 2^31; the block arrays must grow on demand, and a run maps to a block by search over `blockStart[]`, not by `i >> 10`.
- **Do not "simplify" the union by leaving it un-clipped.** Contested cells are resolved last-observation-wins by clipping the incoming run, which is what makes the union a partition. Overlapping runs break `runContaining` - a floor-by-key search lands on a short run starting later and misses the longer one that covers the location - so `biomeAt` returns `null` on exactly the contested cells. Clipping also keeps `getEffectiveGoodCount()` from double-counting them.
- **Dense, name-sorted biome ids silently defeat per-block copy-on-write.** Assigning slots by sorting the biomes present at rebuild time renumbers every existing run's id the first time a new biome is observed, invalidating all blocks. Ids must carry over from the previously published name table and only append. It measured as zero reuse with nothing failing; the identity assertion in `MemoryShapeTest.testUnionRebuildSharesUnchangedBlocks` is what caught it, where an allocation-delta assertion would have passed on noise.
- **Test helpers that reflect into internals are format coupling.** `BiomeMenuSourceTest` seeded coverage via `biomePrefixSumsCache`, so deleting that field broke six tests unrelated to the change. Seed through `addBiomeLocation` + a rebuild.

One measurement worth keeping because it is counter-intuitive and cannot be re-derived from the tree: **biome run count tracks area, not biome boundary length** (exponent 1.99-2.02), because `Square.xzToLocation` maps to concentric rings, so a patch spanning many rings yields one run *per ring*. Every `.bin` in this repo is a toy test fixture (largest: 10 biomes, 13 runs), so any sizing question needs `scripts/analyze_memoryshape_bin.py --simulate` or a real scan.
