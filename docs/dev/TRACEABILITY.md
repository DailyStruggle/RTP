...
| REQ-RTP-F-002 | Geometry (Circle/Square/Rect) | DESIGN.md section 3 | `Circle`, `Square`, `Rectangle` | `MemoryShapeTest` |
| REQ-RTP-F-003 | Distributions (Flat/Normal) | DESIGN.md section 3 | `Circle_Normal`, `Square_Normal` | `MemoryShapeTest` |
| REQ-RTP-F-004 | Multi-region management | DESIGN.md section 5 | `Region`, `RegionQueueManager` | `RegionConfigLoaderTest` |
| REQ-RTP-F-005 | O(log n) complexity | DESIGN.md section 3 | `MemoryShape` | `MemoryShapeTest` |
| REQ-RTP-F-006 | No unbounded rerolling | DESIGN.md section 3 | `MemoryShape` | `RegionPipelineTest` |
| REQ-RTP-F-007 | Uniform distribution | DESIGN.md section 3 | `Circle`, `Square` | `DeterministicShapeTest` |
| REQ-RTP-F-008 | Non-blocking execution | DESIGN.md section 1 | `RTPTaskPipe`, adapters | `RTPArchitectureTest` |
| REQ-RTP-F-009 | Redundancy elimination | DESIGN.md section 3 | `MemoryTracker` | `MemoryShapeTest` |
...
| REQ-RTP-S-002 | No chunk leaks | DESIGN.md section 6 | `ChunkReservation`, `MemoryTracker` | `ChunkTicketLifecycleTest` |
...
| REQ-RTP-S-004 | No silent failure | DESIGN.md section 1 | TeleportPipelineTask; `GlobalRegionVerifiers` (a throwing sync *or* async region verifier is logged at WARNING and the location is rejected/fail-safe, never silently accepted) | FailureModeTest, RegionPipelineTest, `ReqRtpS004VerifierFailSafeTest` |
| REQ-RTP-S-005 | No sync chunk I/O | ADR-015/016, [ADR-077](../adr/ADR-077-multi-format-region-support.md) | RTPTaskPipe, adapters, `RegionFileReader`, `AnvilReader`, `LinearRegionReader`, `RegionFileResolver` | AnvilPrefilterTest, LinearRegionReaderTest, RTPArchitectureTest |
...
| REQ-CORE-F-009 | L3 backlog cache (order-preserving FIFO; head-blocking promotion on `UNVERIFIED`; head-drop on `INVALIDATED`; one `.mca` bin verified per `Region.execute()` pulse) | DESIGN.md section 1.1, [ADR-028](../adr/ADR-028-l3-backlog-cache.md) | `BacklogLocationBuffer`, `WorldBacklogBinIndex`, `RegionQueueManager.backlogLocations`, `Region.processBacklog` | `BacklogLocationBufferTest`, `WorldBacklogBinIndexTest` |
