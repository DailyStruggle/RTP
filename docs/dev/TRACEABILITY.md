...
| REQ-RTP-S-004 | No silent failure | DESIGN.md §1 | TeleportPipelineTask; `GlobalRegionVerifiers` (a throwing sync *or* async region verifier is logged at WARNING and the location is rejected/fail-safe, never silently accepted) | FailureModeTest, RegionPipelineTest, `ReqRtpS004VerifierFailSafeTest` |
