...
|---|---|---|---|---|
| REQ-RTP-F-001 | 0–2 tick response | DESIGN.md section 1 | `RegionQueueManager` | `SLATest`, `RegionPipelineTest` |
| REQ-RTP-F-002 | Geometry (Circle/Square/Rect) | DESIGN.md section 3, [ADR-085](../adr/ADR-085-spiral-addressed-hilbert-key-space.md), [ADR-001](../adr/ADR-001-archimedean-spiral-1d-mapping.md) | `Circle`, `Square`, `Rectangle`, `Ellipse`, `Polygon`, `SquareOptimizedDualLayer`, `CircleOptimizedDualLayer`, `SegmentedKeyRunTable`, `PointEdgeSelector`; shared selection in `MemoryShape.rand()` (template: `adjustRange` / `sample` / `postProcess`) | `MemoryShapeTest`, `MemoryShapeSelectionParityTest`, `OptimizedShapesRegistrationTest`, `ADR085ProductionIntegrationTest`, `ShapeSubParameterTabCompleteTest`, `SpiralMathPropertyTest`, `MemoryShapeBinArithmeticPropertyTest` |
| REQ-RTP-F-003 | Distributions (Flat/Normal) | DESIGN.md section 3 | `Circle_Normal`, `Square_Normal`; shared gaussian draw in `NormalMemoryShape.sample()` | `MemoryShapeTest`, `MemoryShapeSelectionParityTest`, `NormalDistributionFidelityRegressionTest` |
| REQ-RTP-F-004 | Multi-region management | DESIGN.md section 5 | `Region`, `RegionQueueManager` | `RegionConfigLoaderTest` |
| REQ-RTP-F-005 | O(log n) complexity | DESIGN.md section 3 | `MemoryShape`; `floorRunIndex` (binary-search run lookup behind `isKnownBad` / `causeAt`), `maybeFlushAndRebuild` (batched learned-state merge so `rand()` does not pay a full re-merge per selection), `absorbIntoAdjacentRun` (in-place run extension for marks within `spatialResolution`) | `MemoryShapeTest`, `MemoryShapeAmortizedLearningTest`, `MemoryShapeSelectionThroughputTest` |
| REQ-RTP-F-006 | No unbounded rerolling | DESIGN.md section 3 | `MemoryShape` | `RegionPipelineTest`, `MemoryShapeSelectionThroughputTest` |
| REQ-RTP-F-007 | Uniform distribution | DESIGN.md section 3 | `Circle`, `Square` | `DeterministicShapeTest`, `MemoryShapeSelectionParityTest` |
| REQ-RTP-F-008 | Non-blocking execution | DESIGN.md section 1 | `RTPTaskPipe`, adapters | `RTPArchitectureTest` |
| REQ-RTP-F-009 | Redundancy elimination | DESIGN.md section 3 | `MemoryTracker`; `MemoryShape.absorbIntoAdjacentRun` (a mark already covered by a learned run is discarded rather than re-queued) | `MemoryShapeTest`, `MemoryShapeAmortizedLearningTest` |
...
| REQ-RTP-S-001 | No lethal destination | DESIGN.md section 3 | `SafetyCheck`, `SafetyScan` | `RTPArchitectureTest` (`core_and_api_must_not_depend_on_platform_apis` enforces platform-neutral safety checks without leaked platform states), `SafetyScanTest` |
| REQ-RTP-S-002 | No chunk leaks | DESIGN.md section 6 | `ChunkReservation`, `MemoryTracker`; `RegionQueueManager#demoteToUnkept` (sole hot-to-cold disposal path: closes the reservation before offering the bare coordinate, shared by `releaseToNetworkKept`, `disableLoginCache`, `closePersonalQueue`); `CacheStage`, `SimpleCacheStage`, `RingCacheStage`, `KeyedCacheStage`, `HotSink`, `HotBudgetAllocator` ([ADR-078](../adr/ADR-078-composable-cache-pipeline-stages.md) phase 1 & 3: disposal is the single terminal release path, so overflow, down-size, and teardown all release through one handler; budget allocator moves hot entries with zero ticket leaks); `GroupSubspaceHotSink`, `GroupColdToHotTransition` (ADR-078 phase 6: multi-chunk footprint reservations closed on overflow/failure) | `ChunkTicketLifecycleTest`, `ReqRtpS002PipelineLeakAssertionTest` (full pipeline run, cancellation, failure, and GC sweep leak assertions), `RegionQueueManagerNetworkReservedLifecycleTest` (5 demotion/hand-back ticket-hygiene cases), `CacheStageContractTest`, `KeyedCacheStageTest`, `HotSinkTransferEligibilityTest`, `HotBudgetAllocatorTest`, `GroupSubspaceCacheTest`, `RTPArchitectureTest` (`only_ChunkReservation_and_RTPWorld_may_call_setForceLoaded`, `only_RTPWorld_may_call_setForceLoadedImpl`) |
| REQ-RTP-S-003 | Respect claims | ADR-019, [ADR-058](../adr/ADR-058-region-specific-schematic-paste.md) (schematic-paste footprint guard) | `GlobalRegionVerifiers`; `TeleportPipelineTask#schematicFootprintClear` (region schematic paste runs the whole footprint through the verifier registry and suppresses the paste on any claim intersection, falling back to the default platform) | `ReqRtpS003SchematicFootprintClaimTest`, `RTPArchitectureTest` (`core_and_api_must_not_depend_on_platform_apis` gates claim calls from leaking into core/api) |
| REQ-RTP-S-004 | No silent failure | DESIGN.md section 1 | TeleportPipelineTask; `GlobalRegionVerifiers` (a throwing sync *or* async region verifier is logged at WARNING and the location is rejected/fail-safe, never silently accepted); `StageTransition`, `TransitionOutcome`, `RejectionReason` ([ADR-078](../adr/ADR-078-composable-cache-pipeline-stages.md) phase 1: a declined promotion carries a stated cause instead of a bare empty result); `GroupBacklogToColdTransition`, `GroupColdToHotTransition` (ADR-078 phase 6: explicit `TransitionOutcome` rejections across group cache tiers); `GroupPlacementDispatcher` | FailureModeTest, RegionPipelineTest, `ReqRtpS004VerifierFailSafeTest`, `TransitionOutcomeTest`, `GroupSubspaceCacheTest`, `GroupPlacementDispatcherTest`, `QueueTaskTest`, `RTPArchitectureTest` (`no_print_stack_trace_in_core_or_api`, `no_system_out_or_bukkit_getlogger_in_core_or_api` enforce structured logging via `RTP.log(Level.WARNING, ...)` rather than silent drops or raw stderr dumps) |
| REQ-RTP-S-005 | No sync chunk I/O | ADR-015/016, [ADR-077](../adr/ADR-077-multi-format-region-support.md) | RTPTaskPipe, adapters, `RegionFileReader`, `AnvilReader`, `LinearRegionReader`, `RegionFileResolver`; `HotSink#accepts` / `HotSink#transferEligible`, `HotBudgetAllocator#rebalance` ([ADR-078](../adr/ADR-078-composable-cache-pipeline-stages.md) phase 1 & 3: transfer eligibility and rebalancing operate as pure in-memory rechecks against the entry's already-resident chunk, moving entries between sinks with zero chunk load/unload I/O); `GroupBacklogToColdTransition`, `GroupColdToHotTransition` (ADR-078 phase 6: off-tick candidate screening and async chunk loading) | AnvilPrefilterTest, LinearRegionReaderTest, `RTPArchitectureTest` (`no_blocking_future_calls_in_core_or_api`, `no_thread_instantiation_or_executors_outside_carveouts`, `no_synchronous_chunk_io_on_platform_world`), `ReqRtpMap002NoChunkIoTest`, `HotSinkTransferEligibilityTest`, `HotBudgetAllocatorTest`, `GroupSubspaceCacheTest` |
| REQ-RTP-S-006 | No undefined behaviour on early API access | ARCHITECTURE.md — rtp-api | `RTPAPI.hooks()`, `RTPAPI.teleport()`, `RTPAPI.setServerAccessor()` (contract surface). Shape/vert registration moved to `RTP.addShape()`/`RTP.addVerticalAdjustor()` extension tier ([ADR-051](../adr/ADR-051-two-tier-api-extension-model.md)) | `RTPAPIGuardTest` (write-once guard), `RtpApiTeleportSurfaceTest` (pre-init ISE), `RtpApiMenuDataSurfaceTest` (menu-data reads: pre-init ISE, null-arg rejection, safe non-null degradation), `RTPArchitectureTest` (`core_and_api_must_not_depend_on_platform_apis` prevents early-access platform linkage errors) |
| REQ-RTP-S-007 | Configurable busy and invalid command messages | REQ-RTP-F-013, ARCHITECTURE.md | `ConfigParser`, `MessagesKeys`, `BukkitBaseRTPCmd` | `ReqRtpMenuConcreteCommandsTest`, `ConfigParserLanguageTest`, `RTPArchitectureTest` (`no_system_out_or_bukkit_getlogger_in_core_or_api` guarantees command responses and audits route through configurable messaging and `RTP.log`) |
...
| REQ-RTP-OBS-001 | Non-blocking metrics snapshot (extends REQ-RTP-F-008 to metrics surface; M2 adds `FoliaRegionSample` per-region detail, `RegionQueueRow` and `RegionQueueStatus` per-region queue and cache status) | [METRICS_PLAN.md](METRICS_PLAN.md), [metrics-api-ADR-001](../../metrics-api/docs/adr/metrics-api-ADR-001-module-extraction.md) | `Metrics`, `MetricsSnapshot`, `MetricsBinding`, `FoliaRegionSample`, `MetricsExtension`, `RegionQueueRow`, `RegionQueueStatus` (all in `metrics-api`, `io.github.dailystruggle.metrics.api.*`); `CoreMetrics`, `RTPMetricsExtension` (`rtp-core`) | `ReqRtpObs001CoreMetricsTest`, `ReqRtpObs001MetricsSnapshotTest`, `FoliaRegionSampleTest`, `RegionQueueRowTest`, `ReqRtpObs001RegionQueueStatusTest`, `InfoCmdTest` (Folia-table render/suppress cases) |
...
| REQ-RTP-OBS-003 | Bounded heap / TPS / MSPT sampling cost (M2 extends to Folia per-region samplers and the Fabric / NeoForge server-tick samplers; bStats *Runtime health* charts consume the same snapshot). On the vanilla single-loop runtimes MSPT is sourced from the server's own average in-tick work duration, never from the sleep-padded tick interval (see *Vanilla MSPT Source* in METRICS_PLAN.md) | [METRICS_PLAN.md](METRICS_PLAN.md), [metrics-api-ADR-001](../../metrics-api/docs/adr/metrics-api-ADR-001-module-extraction.md) | `HeapSampler` (`rtp-core`), `PaperMetricsBinding`, `BukkitTpsSampler`, `FoliaMetricsBinding`, `FoliaRegionTpsSampler`, `FabricMetricsBinding`, `NeoForgeMetricsBinding` (platform adapters, implementing `metrics-api` `MetricsBinding`); tick-work sources wired at install: `RTPFabricMod.reflectTickWorkNanos`, `RTPNeoForgeMod` (`MinecraftServer#getAverageTickTimeNanos`); `MetricsBindingDispatcher`, `RTPCostMetricsCharts` (bStats *Runtime health* chart group) | `ReqRtpObs003HeapSamplerTest`, `PaperMetricsBindingTest`, `BukkitTpsSamplerTest`, `FoliaMetricsBindingTest`, `FabricMetricsBindingTest` (incl. `mspt_reports_work_duration_not_tick_interval`, `mspt_decreases_after_a_spike_subsides`), `NeoForgeMetricsBindingTest`, `RuntimeHealthBucketsTest` |
...
| REQ-RTP-OBS-005 | Slow-teleport audit (immediate/unqueued only; opt-out threshold; WARN + cumulative `slowPipelineCount`) | [ADR-053](../adr/ADR-053-pipeline-latency-percentiles-and-slow-teleport-audit.md) section 2a, [METRICS_PLAN.md](METRICS_PLAN.md) | `CoreMetrics.auditImmediateTeleport`/`slowPipelineCount`/`slowPipelineThresholdMs`, `TeleportPipelineTask` (`immediateTeleport` flag + `runCleanup` audit call), `PerformanceKeys.slowPipelineThresholdMs`, `RTPMetricsExtension.slowPipelineCount/slowPipelineThresholdMs`; `InfoCmd` `infoSlowPipeline` | `ReqRtpObs005SlowTeleportAuditTest` |
| REQ-RTP-OBS-006 | Queue-growth audit (edge-triggered on `queueDepth`; opt-in threshold; WARN + cumulative `queueGrowthWarnCount`) | [ADR-053](../adr/ADR-053-pipeline-latency-percentiles-and-slow-teleport-audit.md) section 2b, [METRICS_PLAN.md](METRICS_PLAN.md) | `CoreMetrics.evaluateQueueGrowth`/`queueGrowthWarnCount`/`queueGrowthWarnThreshold`, `PerformanceKeys.queueGrowthWarnThreshold`, `RTPMetricsExtension.queueGrowthWarnCount/queueGrowthWarnThreshold`; `InfoCmd` `infoQueueGrowth` | `ReqRtpObs005SlowTeleportAuditTest` (queue-growth disabled-by-default + snapshot wiring cases) |
...
| REQ-API-ARCH-002 | Non-blocking API | DESIGN.md section 1 | ILocationGenerator | RTPArchitectureTest |
| REQ-API-ARCH-003 | API exception handling | DESIGN.md section 6 | TeleportPipelineTask | RTPAPIGuardTest |
| REQ-API-ARCH-004 | Lock-free reads | DESIGN.md section 1 | ConfigParser | ConfigParserLanguageTest |
...
| Req ID | Summary | Design Ref | Implementing Class | Test(s) |
|---|---|---|---|---|
| REQ-CORE-F-001 | Async queue | DESIGN.md section 1 | RegionQueueManager | FailureModeTest |
...
| REQ-CORE-F-009 | Backlog cache (order-preserving FIFO; head-blocking promotion on `UNVERIFIED`; head-drop on `INVALIDATED`; one `.mca` bin verified per `Region.execute()` pulse). Named `L3` by ADR-028; the vocabulary is retired to hot/cold/backlog by [ADR-078](../adr/ADR-078-composable-cache-pipeline-stages.md) phase 2 | DESIGN.md section 1.1, [ADR-028](../adr/ADR-028-l3-backlog-cache.md) | `BacklogLocationBuffer`, `WorldBacklogBinIndex`, `RegionFileCoord`, `RegionQueueManager.backlogLocations`, `Region.processBacklog` | `BacklogLocationBufferTest`, `WorldBacklogBinIndexTest`, `RegionFileCoordTest`, `CacheNomenclatureGuardTest` (ADR-078 phase 2: production sources and the shipped `rtp-plugin` yml config/locale tree carry no `L1`/`L2`/`L3` tier vocabulary) |
...
| REQ-CORE-ARCH-001 | Lock-free config | DESIGN.md section 1 | FactoryValue | ConfigParserLanguageTest |
| REQ-CORE-ARCH-002 | Data access pattern | DESIGN.md section 1 | FactoryValue.getData() | MultiConfigParserIsolationTest |
| REQ-CORE-ARCH-003 | Fault encapsulation | DESIGN.md section 6 | TeleportPipelineTask | - |
| REQ-CORE-ARCH-004 | runCleanup() | DESIGN.md section 6 | TeleportPipelineTask | - |
| REQ-CORE-ARCH-005 | Pulse maintenance | DESIGN.md section 1 | AsyncTaskProcessing | - |
| REQ-CORE-ARCH-006 | Time-bounded tasks | DESIGN.md section 1 | TimeBoundTaskPipe | - |
| REQ-CORE-ARCH-007 | Tracker registration | DESIGN.md section 6 | MemoryTracker | - |
| REQ-CORE-ARCH-008 | Max lifespan | DESIGN.md section 6 | MemoryTracker | - |
...
| Req ID | Summary | Design Ref | Implementing Class(es) | Test(s) |
|---|---|---|---|---|
| REQ-SPIGOT-F-001 | Synchronous chunk loading | DESIGN.md section 2 — rtp-bukkit | `rtp-bukkit` adapter (Bukkit chunk API) | `BukkitSchedulerImplTest` (`runTask_onPrimaryThread_executesImmediately`, `runTaskAsynchronously_dispatchesTask_andCompletes`) |
| REQ-SPIGOT-F-002 | Rate-limited sync tasks | DESIGN.md section 1 — Bounded Computation Overhead | `TimeBoundTaskPipe` (spigot binding) | Manual verification required (TimeBoundTaskPipe core logic covered by `SLATest`) |
...
| REQ-SPIGOT-ARCH-001 | `addPluginChunkTicket` over `setForceLoaded` | DESIGN.md section 6 — Chunk Allocation Management | `rtp-bukkit` chunk reservation impl | `ChunkTicketLifecycleTest` (`ticket_is_held_during_reservation_and_released_on_close`) |
| REQ-SPIGOT-ARCH-002 | Plugin-owned ticket leak prevention | DESIGN.md section 6 — Chunk Allocation Management | `rtp-bukkit` chunk reservation impl | Manual verification required |
| REQ-SPIGOT-ARCH-003 | `TimeBoundTaskPipe` for main-thread ops | DESIGN.md section 1 — Bounded Computation Overhead | `TimeBoundTaskPipe` | `BukkitSchedulerImplTest` (`runTask_onPrimaryThread_executesImmediately`) |
| REQ-SPIGOT-ARCH-004 | Wall-clock time bounding | DESIGN.md section 1 — Bounded Computation Overhead | `TimeBoundTaskPipe` (nanosecond budget) | `BukkitSchedulerImplTest` (`runTaskTimer_returnsNonNullBukkitTaskHandle`, `cancelTask_preventsScheduledTaskFromRunning`) |
| REQ-SPIGOT-ARCH-005 | Ticket lifecycle pairing | DESIGN.md section 6 — Chunk Allocation Management | `rtp-bukkit` `removePluginChunkTicket` call sites | `ChunkTicketLifecycleTest` (`ticket_is_held_during_reservation_and_released_on_close`, `ticket_is_released_even_when_validator_throws`) |
| REQ-SPIGOT-ARCH-006 | Forced reclamation on disconnect | DESIGN.md section 6 — Orphaned Allocation Recovery | `MemoryTracker` + spigot cleanup listener | Manual verification required |
...
| Req ID | Summary | Design Ref | Implementing Class(es) | Test(s) |
|---|---|---|---|---|
| REQ-PAPER-F-001 | `getChunkAtAsync` for non-blocking load | DESIGN.md section 2 — rtp-paper | `rtp-paper` async chunk impl | — |
| REQ-PAPER-F-002 | Callback/future-only, no sync waits | DESIGN.md section 2 — rtp-paper | `rtp-paper` async chunk impl | `RTPArchitectureTest` (`no_blocking_future_calls_in_core_or_api`) |
| REQ-PAPER-F-003 | Paper-specific API prioritization | DESIGN.md section 2 — rtp-paper | `rtp-paper` adapter | `ServerAccessorImplTest` (`getBiomes_returnsNonEmptyUpperCaseSet` — asserts `Registry.BIOME` path) |
| REQ-PAPER-ARCH-001 | `addPluginChunkTicket` over `setForceLoaded` | DESIGN.md section 6 — Chunk Allocation Management | `rtp-paper` chunk reservation impl | `ChunkTicketLifecycleTest` (`ticket_is_held_during_reservation_and_released_on_close`) |
| REQ-PAPER-ARCH-002 | Plugin-owned ticket leak prevention | DESIGN.md section 6 — Chunk Allocation Management | `rtp-paper` chunk reservation impl | — |
| REQ-PAPER-ARCH-003 | `TimeBoundTaskPipe` for main-thread ops | DESIGN.md section 1 — Bounded Computation Overhead | `TimeBoundTaskPipe` (paper binding) | `ServerAccessorImplTest` (`paperScheduler_runTask_executesOnPrimaryThread`) |
| REQ-PAPER-ARCH-004 | Wall-clock time bounding | DESIGN.md section 1 — Bounded Computation Overhead | `TimeBoundTaskPipe` (nanosecond budget) | — |
| REQ-PAPER-ARCH-005 | Ticket lifecycle pairing | DESIGN.md section 6 — Chunk Allocation Management | `rtp-paper` `removePluginChunkTicket` call sites | `ChunkTicketLifecycleTest` (`ticket_is_held_during_reservation_and_released_on_close`, `ticket_is_released_even_when_validator_throws`) |
| REQ-PAPER-ARCH-006 | Forced reclamation on disconnect | DESIGN.md section 6 — Orphaned Allocation Recovery | `MemoryTracker` + paper cleanup listener | — |
...
| Req ID | Summary | Design Ref | Implementing Class(es) | Test(s) |
|---|---|---|---|---|
| REQ-FOLIA-F-001 | Regional thread isolation | DESIGN.md section 2 — rtp-folia | `rtp-folia` regional task dispatcher | — |
| REQ-FOLIA-F-002 | `RegionScheduler` / `EntityScheduler` usage | DESIGN.md section 2 — rtp-folia | `rtp-folia` scheduler impl | `RTPArchitectureTest` (`scheduler_implementations_must_not_reside_in_core`) |
| REQ-FOLIA-F-003 | Concurrent state mutation safety | DESIGN.md section 2 — rtp-folia | `rtp-folia` adapter, `LockFreeLocationBuffer`, `RegionQueueManager` | `LockFreeLocationBufferConcurrencyStressTest`, `RegionQueueManagerConcurrencyStressTest` |
| REQ-FOLIA-F-004 | Cross-region teleport pipelining | DESIGN.md section 2 — rtp-folia | `TeleportPipelineTask` + folia scheduler | — |
| REQ-FOLIA-ARCH-001 | `isOwnedByCurrentRegion` pre-check | DESIGN.md section 2 — rtp-folia | `rtp-folia` regional dispatcher | — |
| REQ-FOLIA-ARCH-002 | Immediate execution if region owned | DESIGN.md section 2 — rtp-folia | `rtp-folia` regional dispatcher | — |
| REQ-FOLIA-ARCH-003 | `RegionScheduler.run()` only when not owned | DESIGN.md section 2 — rtp-folia | `rtp-folia` regional dispatcher | — |
| REQ-FOLIA-ARCH-004 | No `System.nanoTime()` in regional threads | DESIGN.md section 2 — rtp-folia | `rtp-folia` (uses `CountBoundTaskPipe`) | — |
| REQ-FOLIA-ARCH-005 | `CountBoundTaskPipe` for iterative ops | DESIGN.md section 2 — rtp-folia | `rtp-folia` queue replenishment | — |
| REQ-FOLIA-ARCH-006 | Count-based task slicing | DESIGN.md section 2 — rtp-folia | `rtp-folia` `CountBoundTaskPipe` binding | — |
| REQ-FOLIA-ARCH-007 | No Vault calls on region threads | DESIGN.md section 2 — rtp-folia | `rtp-folia` economy delegation | — |
| REQ-FOLIA-ARCH-008 | Economy delegated to `GlobalRegionScheduler` | DESIGN.md section 2 — rtp-folia | `rtp-folia` economy delegation | — |
| REQ-FOLIA-ARCH-009 | Async economy result pipelining | DESIGN.md section 2 — rtp-folia | `rtp-folia` economy delegation | — |
| REQ-FOLIA-ARCH-010 | Native Folia async chunk APIs | DESIGN.md section 2 — rtp-folia | `rtp-folia` chunk impl | — |
...
NeoForge is in-scope per [ADR-033](../adr/ADR-033-neoforge-platform-in-scope.md) and [rtp-neoforge-ADR-001](../../platforms/rtp-neoforge/docs/adr/rtp-neoforge-ADR-001-platform-in-scope.md) (Accepted). Per ADR-033 section 3.4 the S-005 and S-006 guards are authored as part of the Phase N2 platform-adapter bring-up; both rows below are now **implemented**. Remaining NeoForge functional parity (the `/rtp` round-trip exit gate and the final `-PincludeNeoforge` build) is tracked in [`scratch/CHECKLIST-neoforge-phase-n2.md`](scratch/CHECKLIST-neoforge-phase-n2.md).

| Req ID | Summary | Design Ref | Implementing Class(es) | Test(s) |
|---|---|---|---|---|
| REQ-NEOFORGE-ARCH-S005 | No synchronous chunk I/O on the main thread (`NeoForgeRTPWorld.getChunkAt` returns `CompletableFuture`; anvil-backed `NeoForgeRTPChunk` answers block queries from the decoded view without a live load) | ADR-033 section 3.4; rtp-neoforge-ADR-001 section 6; REQ-RTP-S-005 | `NeoForgeRTPChunk` (anvil dual-mode dispatch); `NeoForgeVersionAdapter#requestFullChunkAsync` (async path) | `ReqRtpNeoforgeS005ChunkLoadingTest` (6 tests: anvil-mode flags, null-view rejection, view-routed block queries, isSafe short-circuit + delegation, keep/unload no-ops) |
| REQ-NEOFORGE-ARCH-S006 | Fail-loud on early API access (`IllegalStateException`, never null / no-op) pre-init | ADR-033 section 3.4; rtp-neoforge-ADR-001 section 6; REQ-RTP-S-006 | `NeoForgeVersionAdapterRegistry#require`; `NeoForgeServerAccessor#getLocationGenerator` | `ReqRtpNeoforgeS006ApiBeforeCoreTest` (3 tests: registry not-installed, `require()` throws, `getLocationGenerator()` throws pre-core) |
...
| Req ID | Summary | Design Ref | Implementing Class(es) | Test(s) |
|---|---|---|---|---|
| REQ-RTP-PROXY-001 | Adapter SPI Conformance | ADR-036 section 7; `platforms/rtp-proxy/REQUIREMENTS.md` | — (unimplemented) | — |
| REQ-RTP-PROXY-002 | No World State | ADR-036 section 2 Non-Goals; `platforms/rtp-proxy/REQUIREMENTS.md` | — (unimplemented) | — |
| REQ-RTP-PROXY-003 | Non-Blocking Event Loop | ADR-036 section 7; `platforms/rtp-proxy/REQUIREMENTS.md` | — (unimplemented) | — |
| REQ-RTP-PROXY-004 | Reservation Claim Idempotency | ADR-036 section 5; `platforms/rtp-proxy/REQUIREMENTS.md` | `InMemoryNetworkStateBinding` (reference) | `InMemoryNetworkStateBindingTest` (claim atomicity / per-player idempotency) |
| REQ-RTP-PROXY-005 | Proxy Heartbeat Publication | ADR-036 section 5; `platforms/rtp-proxy/REQUIREMENTS.md` | — (unimplemented) | — |
| REQ-RTP-PROXY-006 | Configurable Messaging | ADR-036 section 3 Locked-In; `platforms/rtp-proxy/REQUIREMENTS.md` | — (unimplemented) | — |
| REQ-RTP-PROXY-007 | Authenticated Transport | ADR-036 section 3 D4; `rtp-proxy-ADR-010` (Proposed) | — (unimplemented) | — |
| REQ-RTP-PROXY-008 | Disabled-Mode No-Op | ADR-036 section 1 (REQ-RTP-NET-002 release gate) | — (unimplemented) | — |
| REQ-RTP-PROXY-009 | Single-Artifact Activation | ADR-036 section 7; `rtp-fabric-ADR-002` precedent | — (unimplemented) | — |
| REQ-RTP-PROXY-010 | Adapter Isolation | ADR-036 section 7 | — (unimplemented) | — |
| REQ-RTP-PROXY-011 | Bounded Tab-Completion Cost | ADR-036 section 3 Locked-In (Brigadier reuse) | — (unimplemented) | — |
...
| REQ-RTP-PROXY-COMMON-008 | Hot-Spot Counter Locality | ADR-036 section 5 Local state is advisory | — (unimplemented) | — |
...
| REQ-RTP-PROXY-VELOCITY-004 | Player Session Continuity | ADR-036 section 1 Headline Goals | — (unimplemented) | — |
| REQ-RTP-PROXY-VELOCITY-005 | Tab-Completion Routing | REQ-RTP-PROXY-011; `commands-api-ADR-001` | — (unimplemented) | — |
| REQ-RTP-PROXY-VELOCITY-006 | Telemetry Scheduling | ADR-036 section 5 Proxy heartbeat | — (unimplemented) | — |
...
| REQ-RTP-PROXY-BUNGEE-004 | Scheduler Discipline | ADR-036 section 7 Adapter isolation | — (unimplemented) | — |
| REQ-RTP-PROXY-BUNGEE-005 | Plugin-Message Transport Eligibility | ADR-036 section 3 D2 (dev-only) | — (unimplemented) | — |
