...
| REQ-RTP-S-002 | No chunk leaks | DESIGN.md section 6 | `ChunkReservation`, `MemoryTracker`; `RegionQueueManager#demoteToUnkept` (sole hot-to-cold disposal path: closes the reservation before offering the bare coordinate, shared by `releaseToNetworkKept`, `disableLoginCache`, `closePersonalQueue`); `CacheStage`, `SimpleCacheStage`, `KeyedCacheStage`, `HotSink` ([ADR-078](../adr/ADR-078-composable-cache-pipeline-stages.md) phase 1: disposal is the single terminal release path, so overflow, down-size, and teardown all release through one handler) | `ChunkTicketLifecycleTest`, `RegionQueueManagerNetworkReservedLifecycleTest` (5 demotion/hand-back ticket-hygiene cases), `CacheStageContractTest`, `KeyedCacheStageTest`, `HotSinkTransferEligibilityTest` |
...
| REQ-RTP-S-004 | No silent failure | DESIGN.md section 1 | TeleportPipelineTask; `GlobalRegionVerifiers` (a throwing sync *or* async region verifier is logged at WARNING and the location is rejected/fail-safe, never silently accepted); `StageTransition`, `TransitionOutcome`, `RejectionReason` ([ADR-078](../adr/ADR-078-composable-cache-pipeline-stages.md) phase 1: a declined promotion carries a stated cause instead of a bare empty result) | FailureModeTest, RegionPipelineTest, `ReqRtpS004VerifierFailSafeTest`, `TransitionOutcomeTest` |
| REQ-RTP-S-005 | No sync chunk I/O | ADR-015/016, [ADR-077](../adr/ADR-077-multi-format-region-support.md) | RTPTaskPipe, adapters, `RegionFileReader`, `AnvilReader`, `LinearRegionReader`, `RegionFileResolver`; `HotSink#accepts` / `HotSink#transferEligible` ([ADR-078](../adr/ADR-078-composable-cache-pipeline-stages.md) phase 1: transfer eligibility is decided by an in-memory recheck against the entry's already-resident chunk and fails closed rather than loading) | AnvilPrefilterTest, LinearRegionReaderTest, RTPArchitectureTest, `HotSinkTransferEligibilityTest` |
