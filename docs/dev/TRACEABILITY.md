...
| REQ-RTP-F-002 | Geometry (Circle/Square/Rect) | DESIGN.md section 3, [ADR-085](../adr/ADR-085-spiral-addressed-hilbert-key-space.md) | `Circle`, `Square`, `Rectangle`, `Ellipse`, `Polygon`, `SquareOptimizedDualLayer`, `CircleOptimizedDualLayer`, `SegmentedKeyRunTable`, `PointEdgeSelector`; shared selection in `MemoryShape.rand()` (template: `adjustRange` / `sample` / `postProcess`) | `MemoryShapeTest`, `MemoryShapeSelectionParityTest`, `OptimizedShapesRegistrationTest`, `ADR085ProductionIntegrationTest`, `ShapeSubParameterTabCompleteTest` |
...
| REQ-FOLIA-F-003 | Concurrent state mutation safety | DESIGN.md section 2 — rtp-folia | `rtp-folia` adapter, `LockFreeLocationBuffer` | — |
