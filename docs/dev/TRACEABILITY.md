...
All rows below are **unimplemented**; the multi-server / proxy subsystem is governed by the ratified umbrella [ADR-036](../adr/ADR-036-network-mode-multi-server-multi-proxy.md) (Accepted 2026-05-14) and ten subproject ADRs under [`rtp-proxy/docs/adr/`](../../rtp-proxy/docs/adr/) (Phase 1 set `rtp-proxy-ADR-001..004` Accepted; `005..010` Proposed). Design source: [`MULTI_SERVER_PLAN.md`](MULTI_SERVER_PLAN.md). Rows exist so implementing classes and tests can be filled in as Phase 1+ work lands.

| Req ID | Summary | Design Ref | Implementing Class(es) | Test(s) |
|---|---|---|---|---|
| REQ-RTP-NET-001 | Optional network mode (disabled by default) | MULTI_SERVER_PLAN.md � *Config Surface*; ADR-036 | � (unimplemented) | � |
| REQ-RTP-NET-002 | Behavioural parity when disabled | MULTI_SERVER_PLAN.md � *Non-Goals (v1)*, *Phase 1 no-op test*; ADR-036 | � (unimplemented) | � |
...
| REQ-RTP-NET-013 | Multi-flavour persistence compatibility (any shipped SQL accessor) | MULTI_SERVER_PLAN.md � *Storage � Reuse `AbstractSQLDatabaseAccessor`*; ADR-036 | � (unimplemented) | � |
...
| REQ-RTP-PROXY-COMMON-005 | Trigger Source Plurality | ADR-036 §3 D1 | — (unimplemented) | — |
...
All rows below are **unimplemented**. Subproject ADRs `rtp-proxy-ADR-006-velocity-bootstrap` (Proposed; Phase 2).

| Req ID | Summary | Design Ref | Implementing Class(es) | Test(s) |
|---|---|---|---|---|
| REQ-RTP-PROXY-VELOCITY-001 | Velocity Runtime | `rtp-proxy-ADR-006` (Proposed) | — (unimplemented) | — |
| REQ-RTP-PROXY-VELOCITY-002 | Brigadier Command Hosting | `commands-api-ADR-001`; ADR-036 §3 Locked-In | — (unimplemented) | — |
| REQ-RTP-PROXY-VELOCITY-003 | Server Rewrite via `ServerPreConnectEvent` | `rtp-proxy-ADR-006`; MULTI_SERVER_PLAN.md *Coordinate Resolution Timing* | — (unimplemented) | — |
