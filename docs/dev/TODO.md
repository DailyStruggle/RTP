...
- [ ] **Deprecate the addon.** `addons/RTP_Glide` still ships unchanged (`plugin.yml` → `io.github.dailystruggle.rtp_glide.RTP_Glide`) and is not yet marked deprecated. Add a one-release migration note pointing operators at the `GLIDE` effect, remove after the deprecation window, and update [`EXTERNAL_HOOKS.md`](EXTERNAL_HOOKS.md) if the addon registered any hook (it currently does not — the events are addon-local).
