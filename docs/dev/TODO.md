...
- [ ] **`BookMenuRenderer` (Paper / Folia / Spigot).** Adventure `Book` on Paper / Folia; `Player#openBook(ItemStack)` on Spigot with the 1.20.5 / 1.21 component-shift handled in the per-version `rtp-bukkit-v*` subprojects per [ADR-010](../adr/ADR-010-versioned-platform-adapter-submodules.md).
...
- [ ] **Re-render `rtp test full` diagnostic block as a book page.** `TestFullCmd#finishSweep` currently prints a plain-text per-subcommand PASS/FAIL table, a one-line `MetricsSnapshot.toString()` dump, and the legacy `total-audited-warnings` footer (landed 2026-05-16, METRICS_PLAN.md §`rtp test full`, CHECKLIST-metrics-and-multiserver.md row B10). Once the book renderer (this section's `BookMenuRenderer` item) is stable, port that same data into a `MenuModel` / `MenuPage` so the operator gets one clickable per-subcommand row plus a metrics-snapshot page. Plain-text path stays as the chat-renderer fallback per ADR-035. No change to the underlying audit semantics — only the rendering layer.
- [x] **Phase-M1 B → C devstack-smoke gate — closed 2026-05-16 (late session).** Paper 26.1.2 transcripts at 23:53 (pre-reload) and 23:54 (post-`/reload`) both produced `12/14 passed (85.7%), total-audited-warnings=2` with sane `MetricsSnapshot` values and the `[METRICS] installed PaperMetricsBinding (Bukkit#getTPS detected)` confirmation. Scope amended same day: **Spigot deprioritized to nice-to-have** — the raw-Spigot `BukkitTpsSampler` fallback transcript is no longer a gate requirement; the user will exercise that path at final-results time, and a break there is a follow-up rather than a release blocker. Modern Paper does not re-fire `onDisable`/`onEnable` on Bukkit `/reload`, so the dispatcher's full install/uninstall round-trip is exercised only at server stop+start; the pre/post-`/reload` snapshot equivalence demonstrates the binding survives `/reload` without going stale (operationally sufficient). See `scratch/CHECKLIST-metrics-and-multiserver.md` Section A → B / B → C gate block for full evidence.
