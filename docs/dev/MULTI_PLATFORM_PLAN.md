...
- [~] *(2026-06-03)* **Second carrier `rtp-neoforge-v26_1_R1` (MC 26.1, EARLY/EXPERIMENTAL)** authored: pins MC 26.1.2 / NeoForge 26.1.2.65-beta / ModDevGradle 2.0.141. The 26.x line is fully deobfuscated (Mojmap with native parameter names, Parchment removed) and runs on **Java 25**, so the carrier pins a Java 25 toolchain and is gated under `-PexcludeJdk25` (alongside the JDK-25 Fabric carriers) in `settings.gradle` - a JDK-21-only host still produces a 1.21.x-capable NeoForge jar without it. `V26_1_R1NeoForgeVersionAdapter` is a faithful clone of the 1.21.1 carrier; `RTPNeoForgeMod#installVersionAdapter` routes `26.*` to it; `rtp-plugin` merges its Java 25 bytecode into the unified jar (findProject-guarded). **Not yet runtime-verified:** the NeoForge 26.1 userdev artifacts are network-gated, so the maintainer must run `.\gradlew :rtp-neoforge:rtp-neoforge-v26_1_R1:build` on a network-capable JDK-25 host, verify the Mojmap API surface still matches, and re-pin versions when 26.1 settles. Routing for any rolling 1.21.x-head is unchanged: `startsWith("1.21")` already serves the whole 1.21 line from the `v1_21_R1` carrier.
