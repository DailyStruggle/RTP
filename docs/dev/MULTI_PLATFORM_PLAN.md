...
**Activation gate (ADR-033 Decision §2).** No box below may be ticked, and no `rtp-neoforge/**` module skeleton may land under version control, until the Fabric platform clears its stability bar: no open S-005 violations in `rtp-fabric`, the `FabricServerAccessor.getLocationGenerator` null stub resolved, the Loom dependency resolved, and a green `rtp test full` on at least one shipped MC carrier. As of 2026-05-31 the Fabric Phase 2 acceptance gate is met (Step H), so the remaining gate items are the Fabric Step I/J/K follow-ups; activation also requires the explicit ADR-033 §3 bring-up prerequisites (a D-005 proposal, the subproject ADR, and TRACEABILITY rows) before any code lands. Every box here is therefore **not started** by definition until that gate clears.

**Ownership.** Per ADR-033 / ADR-022, NeoForge bring-up requires a named maintainer who owns the platform end-to-end (build, mappings, CI toolchain, S-00x proofs, ongoing maintenance) before Phase N1 begins. **Owner: unassigned.**

### Phase N0 — Scope unlock & bring-up prerequisites *(ADR-033 §3)*

- [ ] **D-005 proposal** referencing [ADR-033](../adr/ADR-033-neoforge-platform-in-scope.md) and [`NEOFORGE_NOTES.md`](NEOFORGE_NOTES.md), confirming the Fabric activation gate is clear and naming the maintainer.
- [ ] **Subproject ADR** `rtp-neoforge/docs/adr/rtp-neoforge-ADR-001-platform-in-scope.md` mirroring [rtp-fabric-ADR-002](../../platforms/rtp-fabric/docs/adr/rtp-fabric-ADR-002-platform-in-scope.md) (subproject ADR numbering restarts at `001` per AGENTS.md *Self-Updating Protocol*); add a row to the *Subproject ADRs* table in [`docs/adr/README.md`](../adr/README.md).
- [ ] **`rtp-neoforge/REQUIREMENTS.md`** authored, mirroring [`platforms/rtp-fabric/REQUIREMENTS.md`](../../platforms/rtp-fabric/REQUIREMENTS.md): `REQ-NEOFORGE-F-001..0NN` (accessor / world / player / scheduler / single-JAR-or-mod metadata / lifecycle wiring / login reserve) and `REQ-NEOFORGE-ARCH-001..0NN` (no-Bukkit-imports, no-core-pollution, build-plugin scope, S-005 async chunk loading, S-004 failure attribution, memory hygiene, S-006 fail-loud + pre-init guard, config dir resolution, DB delegation), restating the Fabric requirements against NeoForge entry-point / event-bus / toolchain surfaces.
- [ ] **Build-toolchain decision** captured in the subproject ADR: NeoGradle vs. ModDevGradle (lean ModDevGradle per `NEOFORGE_NOTES.md` §8); confirm the Java 21+ toolchain (REQ-RTP-SYS-001) and that `run_test` / IntelliJ `.run` configs can drive a NeoForge dev server.
- [ ] **Obf/unobf carrier decision**: NeoForge is Mojmap-at-runtime on 1.20.4+, so most likely no obf carrier is needed, but the per-version structural split (NM-typed surfaces isolated from `rtp-core` / `rtp-api`) is still required (`NEOFORGE_NOTES.md` §5).
