...
- [ ] **Step I — Menu Framework Parity *(NEW, 2026-05-22)*** — the menu rollout under [ADR-035](../adr/ADR-035-interactive-menus-book-first.md) and [ADR-044](../adr/ADR-044-command-tree-menu-reflector.md) shipped its `rtp-api` model and `rtp-core` reflectors as platform-agnostic; the renderer layer was Paper-only. Multi-session rollout (Path B per maintainer 2026-05-24: extract the platform-neutral wiring before duplicating onto Fabric, so both platforms benefit). Sub-items:
    - [x] **Renderer decision ADR** *(drafted 2026-05-22 as [rtp-fabric-ADR-012](../../rtp-fabric/docs/adr/rtp-fabric-ADR-012-menu-renderer-parity.md) Proposed; chose chat-first first, book renderer follow-up for 1.21+ carriers per 2026-05-24 maintainer decision to un-defer the book renderer).*
    ...
    - [ ] **Session 2** — `sendMessageWithRunCommand` SPI extension on `RTPServerAccessor` (existing `sendMessage(...hover, click, ...)` hard-wires `ClickEvent.SUGGEST_COMMAND` per `FabricServerAccessor:720` / Bukkit equivalent — fine for `/rtp info`, wrong for menu fragment clicks where every tap must auto-dispatch). Bukkit + Fabric impls. Then platform-neutral `ChatMenuRenderer` in `rtp-core` (walks `MenuModel.pages() -> page.lines() -> line.fragments()`, calls `RTP.serverAccessor.sendMessageWithRunCommand(...)` per fragment, only platform-coupling is the SPI). `RTPFabricMod` installs `ChatMenuRenderer` against `MenuRendererRegistry`; `RTPCmdFabricRoot` calls `MenuWiringSupport.attachTo(this, fabricBindings)` with a `FabricEffectivePermissionsResolver`-backed probe (per [rtp-fabric-ADR-011](../../rtp-fabric/docs/adr/rtp-fabric-ADR-011-effective-permissions-enumeration.md)) and a null `anvilOpener` (Fabric uses a chat-prompt callback substitute for `PromptAnvilInput` per ADR-012 §3).
    - [ ] **Session 3** — book renderer un-defer (per maintainer 2026-05-24, in lieu of ADR-012 §4's deferral). 1.21+ carriers only (`v1_21_R1`, `v1_21_R5`, `v1_21_R11`, `v26_1_R1`); 1.20.x falls back to the chat renderer. Per-carrier `FabricBookMenuRenderer` builds a written-book `ItemStack` via the version's `WrittenBookContent` data component, sends `ClientboundOpenBookPacket`, and reverts the inventory slot — book content reuses the same `MenuModel` → component translation already living in `BookMenuRenderer.toClickEvent` so divergence is one method.
...
- [ ] **Step J — Network Mode Backend Parity *(NEW, 2026-05-22; lift-to-core path adopted 2026-05-23)*** — `MULTI_SERVER_PLAN.md` Phase 1 SPI lives in `rtp-proxy-common` (platform-agnostic, reachable from Fabric), but the Phase 2 backend integration currently lives under `rtp-plugin/.../bukkit/network/` (`NetworkModeBootstrap` + 12 helpers). A Velocity-routed player arriving on a Fabric backend with a reservation token has no listener to redeem it. Re-audit of the platform-coupling surface on 2026-05-23 showed nine of the thirteen classes have zero `org.bukkit.*` imports and the four that do touch the Bukkit API touch it only for player join/quit subscription + `Bukkit.getPlayer(uuid)` lookup. [ADR-049](../adr/ADR-049-network-mode-platform-neutral-lift.md) (Proposed 2026-05-23) adopts a lift-to-`rtp-core` path that adds a single new SPI primitive (`PlayerLifecycleHook`) on `RTPServerAccessor` and supersedes the parallel-reimplementation path proposed in rtp-fabric-ADR-013. Gated on Step E3 (must teleport locally first) and Step F (reservation-token authorization leans on `hasPermission`). Sub-items:
    - [x] [ADR-049](../adr/ADR-049-network-mode-platform-neutral-lift.md) *(Proposed 2026-05-23, lift-to-core path with `PlayerLifecycleHook` SPI; supersedes [rtp-fabric-ADR-013](../../rtp-fabric/docs/adr/rtp-fabric-ADR-013-network-mode-bootstrap-parity.md). Implementation pending ADR acceptance.)*
    ...
    - [ ] Fabric backend lane in `rtp-proxy/devstack/` already exists (`backend-c`); add an acceptance round-trip in the 2-proxy + 2-backend smoke matrix once the lift lands.
