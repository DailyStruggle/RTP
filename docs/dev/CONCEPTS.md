...
This is why the first teleport after a cold server start might take a moment if the background queue has not yet filled, but subsequent teleports are instant once the queue is populated.
...
Player runs /rtp
 ...
 Dequeue pre-generated location
   ...
   │ (normal case — instant)                            │ (cold start / queue filling)
   ▼                                                    ▼
