...
- **Safety filter:** `safety.yml` accepts flat material names (`LAVA`, `MAGMA_BLOCK`, `CACTUS`, `FIRE`). Unknown materials log one warning, never silently dropped.
- **Cross-server `/rtp`:** `proxy-direct` transport - lightweight TCP socket, no shared database required.
- **Economy:** Vault integration for optional charge-per-teleport.
- **Localization:** multilingual locale switching with bundled `lang/**` packs.
- **Platforms:** Spigot, Paper, Folia, Fabric, NeoForge, Arclight/Mohist - all included.
- **Scan:** built-in off-tick pre-scanner (`/rtp scan`) builds persistent spatial memory so the queue stays full without Chunky.
- *Note: block tag / state-predicate grammar (`#minecraft:leaves`, `OAK_SLAB[waterlogged=true]`, wildcards) and SQL/Redis shared-state backends are not part of this build.*
