...
| **Compute ground distance eagerly for every candidate** | Violates the zero-cost-for-plain-configs invariant of ADR-017 section 4; an unbounded or always-on column walk is a per-candidate hot-path tax for a feature most servers never enable. |
...
- `docs/dev/REQUIREMENTS.md section 3` — REQ-RTP-S-001 (unsafe-block prohibition), REQ-RTP-S-004 (never-silent failure), REQ-RTP-S-005 (no main-thread chunk I/O).
