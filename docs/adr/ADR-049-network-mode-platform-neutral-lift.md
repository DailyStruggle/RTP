...
7. **Move and rewrite the 4 listener classes** (`JoinTriggerSource`, `NetworkWaitlistQuitListener`, `NetworkWaitlistNotifier`, `NetworkWaitlistGuard`) per Decision section 3. Update `RTPCmdBukkit` waitlist-guard call site (`Predicate<CommandSender>` -> `Predicate<RTPCommandSender>`).
