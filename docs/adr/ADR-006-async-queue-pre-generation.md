...
Persistent spatial memory (`MemoryShape`) further guards against runtime chunk-loading costs: as unsafe sectors are recorded, background replenishment skips known-bad ground without loading or ticking candidate chunks repeatedly.
...
- **Positive:**
  ...
  - Persistent spatial memory minimizes unnecessary chunk-loading costs during background replenishment.
...
- **Negative / Trade-offs:**
  ...
  - There is a brief cold-start period after server startup during which the queue may be empty and teleports shall wait.
