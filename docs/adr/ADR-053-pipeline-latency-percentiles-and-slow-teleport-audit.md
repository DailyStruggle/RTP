...
Like section 2a, this is an operational `WARN`-level audit, not an S-004 failure, and is cumulative since process start.
...
- [`docs/dev/REQUIREMENTS.md`](../dev/REQUIREMENTS.md) section 1.8 — REQ-RTP-OBS-004 (percentile readout), REQ-RTP-OBS-005 (slow-teleport audit, immediate/unqueued only), and REQ-RTP-OBS-006 (queue-growth audit).
