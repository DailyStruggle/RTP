...
When a player requests a random teleport, the plugin must select a radial distance `r` from the region centre within the configured `[minRadius, maxRadius]` annulus. This selection is not inherently uniform — different server communities have different goals for *where* players land relative to the inner and outer boundaries of the region.
...
Prior to offering configurable distributions the plugin offered only a single fixed strategy, which could not satisfy use cases 2 and 3 without operators resorting to multiple overlapping regions with different radii.

## Decision

Expose three named probability distributions for radial distance selection, selectable per-region via configuration:
...
`Flat` is the default and is used by the large majority (~90 %+) of operators.

`Normal` was implemented as a requested feature to achieve parity with JakesRTP, which offered a normal distribution. While a specific dominant use-case for it is not defined, it satisfies operators who want a "natural-feeling" distribution that avoids hard edges.
...
- **Negative / Trade-offs:** Three distributions must be maintained and tested. The `Normal` distribution in particular has an unclear primary use-case and was added largely for feature parity; if it is found to be unused it could be deprecated in a future major version.
- **Negative / Trade-offs:** Operators must understand the concept of a probability distribution to choose correctly; documentation in `CONCEPTS.md` and `CONFIGURATION.md` is required to make this accessible.
