...
Marketplace listing metadata (current, for SEO reference):
  ...
  Tagline: "Deterministic, off-tick random teleport engine with spatial memory"
           (Modrinth's short description field still reads "Fast Random Teleportation for
            everyone, configurable and extensible" - update it there to match.)
-->
...
Rig: Ryzen 9 3900X, 16 GiB allocated to the server, one rig for every row. Each competing plugin ran on the latest version it supported at the time of the run, with cooldowns, delays, and countdowns zeroed and queues enabled where the plugin offers them. 60 s warm-up before measurement.
...
**Metrics:** MSPT p99 (worst 1-in-100 main-thread tick in ms, lower better) | Throughput (TP/s, higher better) | Min TPS (lowest TPS observed; 20.00 = no hiccup) | CPU / TP (main-thread CPU per successful teleport).

MSPT p99 leads because throughput saturates: at 1 `/rtp` per gametick the harness itself is the ceiling, so TP/s can tie at the top and tell you nothing. The tail is where the plugins separate.
...
**Q: I'm on NeoForge.**
A: There's a native NeoForge adapter for 1.21.x / 26.x, running the same `rtp-core` engine as every other platform. I test it less than the Bukkit family and Fabric, and broader runtime testing on the 26.x carrier is still ongoing, so if you hit something there it jumps the queue.
