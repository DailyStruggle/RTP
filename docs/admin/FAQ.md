...
No. Run `/rtp reload` after editing any config file. The only exception is plugin.yml entries (e.g., adding a new permission node that another plugin reads at startup) — those require a full restart.
...
Run `/rtp fill` to pre-warm the queue before players join, or increase `cacheCap` in the region config to keep more locations ready.
...
- `mode: NEAREST` is set — this can cluster landings near unblocked edges. Switch to `mode: ACCUMULATE` for even distribution.

### "No safe location found" — what does this mean?

RTP exhausted its `maxAttempts` (set in `performance.yml`) without finding a valid location. Common causes:
1. The region radius is too small relative to the number of excluded biomes.
2. `minY` / `maxY` are too restrictive for the world's terrain (e.g., nether with `maxY: 255`).
...
It controls how precisely the plugin stores "bad location" data. Higher values = finer grid = more memory. Lower values = coarser grid = less memory but more imprecision. The default (`3`) suits most servers. Increase it only if players are reporting they can't land in areas that should be valid.
...
- `syncAllottedTime` — limits how long per tick the synchronous portion of a teleport can take.
...
2. Lower `viewDistanceSelect` in `performance.yml` (default `0` is fine; higher values pre-load more surrounding chunks).
...
Vault alone does not provide an economy — it is a bridge. You also need an economy plugin (e.g., EssentialsX Economy, CMI, or Gringotts). RTP detects both at startup. If economy is still not working after installing both, check the console for a "Vault economy not found" message and restart cleanly.
...
This is the most common first-run confusion. RTP ships with a single region (`default`) that targets the overworld (`world: "[0]"`). **There is no nether or end region out of the box** — you have to create them.
