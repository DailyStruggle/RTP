...
/**
 * Verifies effects-api-ADR-002: type-driven adaptive reading order.
 */
class EffectsApiAdaptiveReadingOrderTest {
    ...
    @Test
    @DisplayName("EffectTarget: constructor accepts handles (ADR-007)")
    void targetAcceptsHandles() {
        PlayerHandle player = org.mockito.Mockito.mock(PlayerHandle.class);
        LocationHandle loc = org.mockito.Mockito.mock(LocationHandle.class);
        EffectTarget target = new EffectTarget(player, loc);
        assertEquals(player, target.player());
        assertEquals(loc, target.location());
    }
    ...
    void potionEffectSkipsKeyOnTypeMismatch() {
        ...
        // Under the unified common PotionKeys
        ...
        assertEquals(Boolean.TRUE, effect.getData().get(PotionEffect.PotionKeys.AMBIENT));
        assertEquals(Boolean.FALSE, effect.getData().get(PotionEffect.PotionKeys.PARTICLES));
        assertEquals(Boolean.TRUE, effect.getData().get(PotionEffect.PotionKeys.ICON));
    ...
    void soundEffectSkipsTypeWhenOnlyNumbersGiven() {
        ...
        assertNotNull(effect.getData().get(SoundEffect.SoundKeys.TYPE), "TYPE default must survive skip");
        assertEquals(50, ((Number) effect.getData().get(SoundEffect.SoundKeys.VOLUME)).intValue());
        assertEquals(200, ((Number) effect.getData().get(SoundEffect.SoundKeys.PITCH)).intValue());
        ...
        assertEquals(2.0f / 100f, ((Number) effect.getData().get(SoundEffect.SoundKeys.DX)).floatValue(), 1e-6);
        assertEquals(3.0f / 100f, ((Number) effect.getData().get(SoundEffect.SoundKeys.DY)).floatValue(), 1e-6);
        assertEquals(4.0f / 100f, ((Number) effect.getData().get(SoundEffect.SoundKeys.DZ)).floatValue(), 1e-6);
        assertTrue(warnings.isEmpty(),
    ...
    void soundEffectFloatTokenSkipsToFloatKey() {
        ...
        assertEquals(100, ((Number) effect.getData().get(SoundEffect.SoundKeys.VOLUME)).intValue(),
                "VOLUME must keep its Integer default — 0.5 didn't parse as Integer");
        assertEquals(100, ((Number) effect.getData().get(SoundEffect.SoundKeys.PITCH)).intValue(),
                "PITCH must keep its Integer default — 0.5 didn't parse as Integer");
        assertEquals(0.5d / 100d, ((Number) effect.getData().get(SoundEffect.SoundKeys.DX)).doubleValue(), 1e-6,
    ...
    void cursorDoesNotRewind() {
        ...
        // PotionKeys {TYPE, DURATION, AMPLIFIER, AMBIENT, PARTICLES, ICON}).
        PotionEffect effect = new PotionEffect(PotionEffectType.BLINDNESS);
        effect.setData("3", "7", "11", "false");

        assertEquals(PotionEffectType.BLINDNESS, effect.getData().get(PotionEffect.PotionKeys.TYPE),
                "TYPE must NOT have been retroactively assigned a later int — cursor doesn't rewind");
        assertEquals(3, ((Number) effect.getData().get(PotionEffect.PotionKeys.DURATION)).intValue(),
                "First int landed on DURATION");
        assertEquals(7, ((Number) effect.getData().get(PotionEffect.PotionKeys.AMPLIFIER)).intValue(),
                "Second int landed on AMPLIFIER");
        assertEquals(Boolean.FALSE, effect.getData().get(PotionEffect.PotionKeys.AMBIENT),
