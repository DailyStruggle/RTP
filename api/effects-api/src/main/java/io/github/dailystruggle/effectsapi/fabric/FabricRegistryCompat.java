...
/**
 * Cross-version shim for {@code Registry#get(ResourceLocation)} drift.
 *
 * <p>MC 1.21.2 (Mojmap) renamed {@code Registry.get(ResourceLocation)} to
 * {@code Registry.getValue(ResourceLocation)} (returning {@code T}); the
 * old {@code get(ResourceLocation)} now returns
...
public final class FabricRegistryCompat {
    ...
    public static <T> T resolve(Registry<T> registry, ResourceLocation key) {
    ...
    private static Resolved probe(Class<?> registryClass) {
        ...
        // 1.21.2+: getValue(ResourceLocation) -> T
        try {
            MethodHandle mh = lookup.findVirtual(registryClass, "getValue",
                    MethodType.methodType(Object.class, ResourceLocation.class));
            return new Resolved(Shape.GET_VALUE, mh);
        } catch (Throwable ignored) { /* fall through */ }
        // Legacy (≤1.21.1): get(ResourceLocation) -> T
        try {
            MethodHandle mh = lookup.findVirtual(registryClass, "get",
                    MethodType.methodType(Object.class, ResourceLocation.class));
            return new Resolved(Shape.LEGACY_GET, mh);
        } catch (Throwable ignored) { /* fall through */ }
        // 1.21.2+: get(ResourceLocation) -> Optional<Holder.Reference<T>>
        try {
            MethodHandle mh = lookup.findVirtual(registryClass, "get",
                    MethodType.methodType(Optional.class, ResourceLocation.class));
        ...
        // ((ResourceLocation) -> Object) is unique enough among Registry
        // methods (only the value-by-key getter takes a single
        // ResourceLocation and returns Object/Optional). Walk the public
        // method list and pick the first match.
        java.lang.reflect.Method legacy = null;   // (ResourceLocation) -> T (Object-assignable, non-primitive)
        java.lang.reflect.Method opt = null;      // (ResourceLocation) -> Optional
        for (java.lang.reflect.Method m : registryClass.getMethods()) {
            Class<?>[] p = m.getParameterTypes();
            if (p.length != 1 || p[0] != ResourceLocation.class) continue;
            if (java.lang.reflect.Modifier.isStatic(m.getModifiers())) continue;
            Class<?> ret = m.getReturnType();
            // Reject primitives (notably containsKey -> boolean) and void;
            // both have a (ResourceLocation) param shape but neither is a
