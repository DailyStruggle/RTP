    ...
    private void placeOnShutdown(Player player, GlideState state) {
        ...
        for (int y = startY; y >= minY; y--) {
            ...
            if (head1.getType().isAir() && head2.getType().isAir()) {
                Location target = new Location(world, x + 0.5, y + 1.0, z + 0.5, loc.getYaw(), loc.getPitch());
        ...
        Location target = new Location(world, x + 0.5, platformY + 2.0, z + 0.5, loc.getYaw(), loc.getPitch());
        player.setGliding(false);
        player.teleport(target);
