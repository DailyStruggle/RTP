...
| `mapsapi.fabric` | Layer 3 — Fabric dispatcher | `rtp-api`, `mapsapi`, Loom-managed MC | Hosts `FabricMapBindingDispatcher` and the NM-typed surfaces consumed by the obf carrier in `platforms/rtp-fabric/rtp-fabric-common/.../maps/` and the unobf carrier in `maps-api-fabric-unobf/`. Lands in Stage 3. |
...
Folia does **not** receive its own subpackage inside `maps-api`. `FoliaMapBinding` lives under `platforms/rtp-folia/rtp-folia-common/.../maps/` because it depends on Folia-specific scheduler types (`Bukkit.getGlobalRegionScheduler()`, `EntityScheduler`) that the Spigot 1.20.1 `compileOnly` dependency does not expose. This mirrors ADR-046's *Layer 3* explicit Folia placement.
...
- [rtp-fabric-ADR-009](../../../platforms/rtp-fabric/docs/adr/rtp-fabric-ADR-009-obf-unobf-common-split.md) — Fabric obf/unobf carrier dispatch precedent.
