 ...
 * {@code performance.yml}, {@code safety.yml} and {@code regions/<id>.yml}.
 * Prefabs are sealed Java constants in {@link PrefabRegistry}; there is no
 * on-disk prefab file format and no admin-authored extensibility in v1 (see
 ...
 * {@code performanceOverlay} / {@code safetyOverlay} / {@code regionOverlays}
 * are touched on apply; everything else is preserved. Nested maps recurse;
 * lists are replaced
 ...
 * @param safetyOverlay      sparse overlay applied on top of {@code safety.yml}; never {@code null}, may be empty.
 *                           Used by void-world prefabs (e.g. {@code oneblock}) to enable the emergency
 *                           landing platform that builds the arrival foothold.
