...
/**
 * Tests for Square shape covering:
 * - getRange() boundary conditions
 * - xzToLocation() all 8 octant branches via squareOct2Coords
...
public class SquareTest {
    ...
    @CsvSource({
        // Octant 1: x>0, z>0, z<x  (theta < 0.125)
        "100,  30",
        // Octant 2: x>0, z>0, z>x  (theta in [0.125, 0.25))
        "30,  100",
        // Octant 3: x<0, z>0, |z|>|x|  (theta in [0.25, 0.375))
        "-30,  100",
        // Octant 4: x<0, z>0, |x|>|z|  (theta in [0.375, 0.5))
        "-100,  30",
        // Octant 5: x<0, z<0, |x|>|z|  (theta in [0.5, 0.625))
        "-100, -30",
        // Octant 6: x<0, z<0, |z|>|x|  (theta in [0.625, 0.75))
        "-30, -100",
        // Octant 7: x>0, z<0, |z|>|x|  (theta in [0.75, 0.875))
        "30, -100",
        // Octant 8: x>0, z<0, |x|>|z|  (theta in [0.875, 1.0))
        "100, -30",
        // On-axis
        "100,   0",
        "0,   100",
    ...
    void locationToXZ_roundTrip_withinRadius(long radius, long cr, long location) {
        ...
        // Decoded point should be within the outer square (Chebyshev distance)
    ...
    void customNameConstructor_setsName() {
        Square shape = new Square("MY_SQUARE");
        assertEquals("MY_SQUARE", shape.name);
    }
}
