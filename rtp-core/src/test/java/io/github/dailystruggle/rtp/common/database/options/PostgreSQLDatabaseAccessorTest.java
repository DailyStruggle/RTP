...
class PostgreSQLDatabaseAccessorTest {
    ...
    static class TestablePostgreSQLAccessor extends AbstractSQLDatabaseAccessor {
        ...
        TestablePostgreSQLAccessor(Connection mockConnection) {
        ...
        @Override
        public String name() { return "jdbc:postgresql://test/testdb"; }
        ...
        /** Exact copy of PostgreSQLDatabaseAccessor.read */
        @Override
        ...
        /** Exact copy of PostgreSQLDatabaseAccessor.write */
        @Override
    ...
    private TestablePostgreSQLAccessor accessor;

    @BeforeEach
    void setUp() throws SQLException {
        RTPTestSetup.install(tempDir.toFile());
        mockConn = mock(Connection.class);
        accessor = new TestablePostgreSQLAccessor(mockConn);
    }
    ...
    void read_rowFound_returnsPopulatedMap() throws SQLException {
        ...
        when(meta.getColumnName(2)).thenReturn("cost");
        when(rs.getObject(1)).thenReturn("uuid-pg");
        when(rs.getObject(2)).thenReturn(5.0);

        Optional<Map<String, Object>> result =
                accessor.read(mockConn, "rtp_teleport_data",
                        new AbstractMap.SimpleEntry<>("senderId", "uuid-pg"));

        assertTrue(result.isPresent());
        assertEquals("uuid-pg", result.get().get("senderId"));
        assertEquals(5.0, result.get().get("cost"));
    }
    ...
    @Test
    void write_teleportDataTable_usesOnConflictUpdate() throws SQLException {
        PreparedStatement ps = mock(PreparedStatement.class);
        when(mockConn.prepareStatement(anyString())).thenReturn(ps);

        Map<DatabaseAccessor.TableObj, DatabaseAccessor.TableObj> pairs = new LinkedHashMap<>();
        pairs.put(new DatabaseAccessor.TableObj("senderId"), new DatabaseAccessor.TableObj("uuid-pg"));
        pairs.put(new DatabaseAccessor.TableObj("time"), new DatabaseAccessor.TableObj(42L));

        accessor.write(mockConn, "rtp_teleport_data", pairs);

        verify(ps).executeUpdate();
        // Verify the SQL contained ON CONFLICT
        verify(mockConn).prepareStatement(argThat(sql ->
                sql.contains("ON CONFLICT") && sql.contains("DO UPDATE SET")));
    }

    @Test
    void write_otherTable_usesPlainInsert() throws SQLException {
        PreparedStatement ps = mock(PreparedStatement.class);
        when(mockConn.prepareStatement(anyString())).thenReturn(ps);
    ...
    void connect_getConnectionThrows_returnsNull() {
        TestablePostgreSQLAccessor bad = new TestablePostgreSQLAccessor(mockConn) {
            @Override
    ...
    @Test
    void getInsertStatement_containsOnConflict() {
        assertTrue(accessor.getInsertStatement().contains("ON CONFLICT"));
    }
}
