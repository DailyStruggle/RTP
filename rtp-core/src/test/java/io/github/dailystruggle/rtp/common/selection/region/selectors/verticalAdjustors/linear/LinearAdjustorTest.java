...
public class LinearAdjustorTest {
    ...
    @Test
    void adjust_nullChunk_returnsFalse() {
        LinearAdjustor adj = buildAdjustor(0, 60, 80);
        io.github.dailystruggle.rtp.api.world.MutableRTPCoords out =
                new io.github.dailystruggle.rtp.api.world.MutableRTPCoords("w", 0, 0, 0);
        assertFalse(adj.adjust(null, out));
    }
    ...
    @Test
    void adjustColumn_nullChunk_returnsNull() {
        LinearAdjustor adj = buildAdjustor(0, 60, 80);
        assertNull(adj.adjustColumn(null, 7, 7));
    }

    @Test
    void adjust_allDirections_succeed() {
        ConfigurableMockChunk chunk = new ConfigurableMockChunk(0, 0, world);
        chunk.setSolidSafe(64);
        chunk.setSolidSafe(70);
        ...
        for (int dir = 0; dir <= 4; dir++) {
            ...
            adj.setRng(new java.util.Random(42));
            RTPCoords res = adj.adjust(chunk);
            assertNotNull(res, "Dir " + dir + " should find safe landing");
            assertTrue(res.y() == 65 || res.y() == 71, "Y should be either 65 or 71");

            RTPCoords colRes = adj.adjustColumn(chunk, 7, 7);
            assertNotNull(colRes, "adjustColumn Dir " + dir + " should find safe landing");
            assertTrue(colRes.y() == 65 || colRes.y() == 71);
        }
    }

    @Test
    void adjust_direction1_topDown_findsHighestFloorFirst() {
        ConfigurableMockChunk chunk = new ConfigurableMockChunk(0, 0, world);
        chunk.setSolidSafe(64);
        chunk.setSolidSafe(74);

        LinearAdjustor adj = buildAdjustor(1, 60, 80);
        RTPCoords res = adj.adjust(chunk);
        assertNotNull(res);
        assertEquals(75, res.y(), "Top down should find 74 floor (y=75) first");
    }

    @Test
    void adjust_direction0_bottomUp_findsLowestFloorFirst() {
        ConfigurableMockChunk chunk = new ConfigurableMockChunk(0, 0, world);
        chunk.setSolidSafe(64);
        chunk.setSolidSafe(74);
        ...
        RTPCoords res = adj.adjust(chunk);
        assertNotNull(res);
        assertEquals(65, res.y(), "Bottom up should find 64 floor (y=65) first");
    }

    @Test
    void adjust_direction2_middleOut_findsNearestToMiddle() {
        ConfigurableMockChunk chunk = new ConfigurableMockChunk(0, 0, world);
        chunk.setSolidSafe(70); // middle is (80+60)/2 = 70. Landing at 71.
        chunk.setSolidSafe(62);

        LinearAdjustor adj = buildAdjustor(2, 60, 80);
        RTPCoords res = adj.adjust(chunk);
        assertNotNull(res);
        assertEquals(71, res.y());
    }

    @Test
    void adjust_direction3_edgesIn_findsNearEdgeFirst() {
        ConfigurableMockChunk chunk = new ConfigurableMockChunk(0, 0, world);
        chunk.setSolidSafe(78); // near top edge. Landing at 79.
        chunk.setSolidSafe(70); // near center

        LinearAdjustor adj = buildAdjustor(3, 60, 80);
        RTPCoords res = adj.adjust(chunk);
        assertNotNull(res);
        assertEquals(79, res.y());
    }
    ...
    void adjustFromProbe_windowReject_and_scanMissReject() {
        ...
        // Probe window narrower than [minY, maxY]
        ...
        // Probe window covering [minY, maxY] but all air -> SCAN_MISS_REJECT
    ...
    @Test
    void adjustFromProbe_successfulLanding_allDirections() {
        for (int dir = 0; dir <= 4; dir++) {
            LinearAdjustor adj = buildAdjustor(dir, 60, 80);
            adj.setRng(new java.util.Random(123));
            ...
            RTPCoords result = adj.adjustFromProbe(probe, "world");
            assertNotNull(result, "Probe scan with dir " + dir + " should succeed");
            assertEquals(64, result.y());
        }
    }

    @Test
    void isGroundSafe_platformDepth_rejectsUnsafeUnderCrust() {
        // Configure safetyRadius = 3 (platformDepth in LinearAdjustor)
        io.github.dailystruggle.rtp.common.RTP.configs.getParser(io.github.dailystruggle.rtp.common.configuration.enums.SafetyKeys.class)
                .set(io.github.dailystruggle.rtp.common.configuration.enums.SafetyKeys.safetyRadius, 3);

        ConfigurableMockChunk chunk = new ConfigurableMockChunk(0, 0, world);
        // Feet at 65 -> y-1=64 (solid safe).
        // If y-2 or y-3 is unsafe (marked solid), isGroundSafe must reject!
        chunk.setSolidSafe(64);
        chunk.setSolid(63); // marked as solid (which makes isSafe return false)

        LinearAdjustor adj = buildAdjustor(0, 60, 80);
        assertNull(adj.adjust(chunk), "Crust with unsafe block below within platformDepth must be rejected");

        // Now remove y=63 from solid and mark it solidSafe
        ConfigurableMockChunk safeChunk = new ConfigurableMockChunk(0, 0, world);
        safeChunk.setSolidSafe(64);
        safeChunk.setSolidSafe(63);
        safeChunk.setSolidSafe(62);
        assertNotNull(adj.adjust(safeChunk), "Deep safe ground must be accepted");
    }

    @Test
    void readSafetySnapshot_unsafeBlocksAndTagStripping() {
        java.util.List<String> list = java.util.Arrays.asList("minecraft:lava", "water", "#minecraft:beds", "stone[waterlogged=true]");
        io.github.dailystruggle.rtp.common.RTP.configs.getParser(io.github.dailystruggle.rtp.common.configuration.enums.BlocksKeys.class)
                .set(io.github.dailystruggle.rtp.common.configuration.enums.BlocksKeys.unsafeBlocks, list);

        // Verify probe recognizes canonicalized names
        LinearAdjustor adj = buildAdjustor(0, 60, 80);
        io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe probe =
                new io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe(0, 0, 0, 128);
        probe.setSolidRange(0, 63);
        probe.setAirRange(64, 128);
        // Put lava at candidate feet cell (64)
        probe.withColumnBlock(7, 7, 64, "minecraft:lava");

        // The probe should reject column with lava
        RTPCoords result = adj.adjustFromProbe(probe, "world");
        // Center column rejected; it may find fallback or fail
        if (result != null) {
