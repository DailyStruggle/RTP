...
public class FixedAdjustorTest {
  ...
  @Test
  void keys_containsExpectedEnumKeys() {
    FixedAdjustor adj = buildAdjustor(64);
    assertTrue(adj.keys().contains("y"));
    assertFalse(adj.keys().isEmpty());
  }

  @Test
  void adjust_worldBoundsClamping_rejectsWhenOutOfBounds() {
    ConfigurableMockChunk chunk = new ConfigurableMockChunk(0, 0, world); // world max height is 256
    int worldMax = world.getMaxHeight();

    // Exactly at worldMax -> reject
    FixedAdjustor adjAtMax = buildAdjustor(worldMax);
    assertNull(adjAtMax.adjust(chunk));

    // Greater than worldMax -> reject
    FixedAdjustor adjAboveMax = buildAdjustor(worldMax + 5);
    assertNull(adjAboveMax.adjust(chunk));

    // At worldMax - 1: y is valid, but y+1 == worldMax -> head out of bounds -> reject
    FixedAdjustor adjHeadAtMax = buildAdjustor(worldMax - 1);
    assertNull(adjHeadAtMax.adjust(chunk));

    // At worldMax - 2: y < worldMax and y+1 < worldMax -> valid!
    FixedAdjustor adjJustInside = buildAdjustor(worldMax - 2);
    assertNotNull(adjJustInside.adjust(chunk));
  }

  @Test
  void adjustFromProbe_andProbeWithReason_validAndWindowMismatch() {
    FixedAdjustor adj = buildAdjustor(64);

    // Matching window, all air
    io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe probeOk =
        new io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe(1, 2, 0, 256)
            .withDefaultBlock("minecraft:air");

    RTPCoords coords = adj.adjustFromProbe(probeOk, "test_world");
    assertNotNull(coords);
    assertEquals(64, coords.y());
    assertEquals((1 << 4) + 7, coords.x());
    assertEquals((2 << 4) + 7, coords.z());

    VerticalAdjustor.AdjustResult okResult = adj.adjustFromProbeWithReason(probeOk, "test_world");
    assertEquals(VerticalAdjustor.ProbeRejectReason.NONE, okResult.reason());
    assertEquals(coords, okResult.picked());

    // Window reject: probe minY > y (e.g. minY=65, y=64)
    io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe probeHigh =
        new io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe(1, 2, 65, 256)
            .withDefaultBlock("minecraft:air");
    assertEquals(VerticalAdjustor.ProbeRejectReason.WINDOW, adj.adjustFromProbeWithReason(probeHigh, "test_world").reason());

    // Window reject: probe maxY < y+1 (e.g. maxY=64, y=64 -> y+1=65 > maxY)
    io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe probeLow =
        new io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe(1, 2, 0, 64)
            .withDefaultBlock("minecraft:air");
    assertEquals(VerticalAdjustor.ProbeRejectReason.WINDOW, adj.adjustFromProbeWithReason(probeLow, "test_world").reason());

    // Scan miss: destination cell solid
    io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe probeSolidY =
        new io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe(1, 2, 0, 256)
            .withDefaultBlock("minecraft:air")
            .withColumnBlock(7, 7, 64, "minecraft:stone");
    assertEquals(VerticalAdjustor.ProbeRejectReason.SCAN_MISS, adj.adjustFromProbeWithReason(probeSolidY, "test_world").reason());

    // Scan miss: head cell solid
    io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe probeSolidHead =
        new io.github.dailystruggle.rtp.common.selection.region.selectors.verticalAdjustors.FakeChunkColumnProbe(1, 2, 0, 256)
            .withDefaultBlock("minecraft:air")
            .withColumnBlock(7, 7, 65, "minecraft:stone");
    assertEquals(VerticalAdjustor.ProbeRejectReason.SCAN_MISS, adj.adjustFromProbeWithReason(probeSolidHead, "test_world").reason());
  }

  @Test
  void testPlacement_checksAllVerifiers() {
    java.util.List<java.util.function.Predicate<RTPCoords>> verifiers = new ArrayList<>();
    verifiers.add(c -> c.y() > 0);
    verifiers.add(c -> c.y() < 100);

    FixedAdjustor adj = new FixedAdjustor(verifiers);
    adj.set(FixedAdjustorKeys.y, 50L);

    assertTrue(adj.testPlacement(new RTPCoords("test_world", 0, 50, 0)));
    assertFalse(adj.testPlacement(new RTPCoords("test_world", 0, -5, 0)));
    assertFalse(adj.testPlacement(new RTPCoords("test_world", 0, 150, 0)));
  }
}
