...
public class ChunkyBorderChecker {
  ...
  /** Load ChunkyBorder integration and register its world border function */
  public static void loadChunky() {
    ...
    if (chunkyBorder != null) {
      RTP.serverAccessor.setWorldBorderFunction(
          worldName -> {
              ...
              vanillaBorder =
                  new WorldBorder(
                      () -> {
                        ...
                        square.set(
                            GenericMemoryShapeParams.radius,
                            ((long) (worldBorder.getSize() * 0.9d)) / 16);
                        square.set(GenericMemoryShapeParams.centerRadius, 0L);
                        square.set(
                            GenericMemoryShapeParams.centerX,
                            worldBorder.getCenter().getBlockX() / 16);
                        square.set(
                            GenericMemoryShapeParams.centerZ,
                            worldBorder.getCenter().getBlockZ() / 16);
                      ...
                      rtpLocation -> {
                        if (RTP.serverAccessor.getServerIntVersion() > 10)
                          return worldBorder.isInside(
                              new Location(
                                  world, rtpLocation.x(), rtpLocation.y(), rtpLocation.z()));
            ...
            try {
              Optional<BorderData> borderDataOptional =
                  ChunkyBorderProvider.get().getBorder(worldName);
              if (borderDataOptional.isPresent()) {
                ...
                if (!value.myClass.equals(RectangleParams.class))
                  throw new IllegalStateException(
                      "chunky shape ` "
                          + "chunky_"
                          + border.name()
                          + " ` not using rectangle as its superclass");
                ...
                chunkyBorder =
                    new WorldBorder(
                        () -> finalShape,
                        rtpLocation ->
                            border.isBounding(rtpLocation.x() / 16.0, rtpLocation.z() / 16.0));
              }
