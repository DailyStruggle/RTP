...
public final class RTPBukkitPlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if ( instance == null ) {
            ...
            try {
                RTP.serverAccessor = (io.github.dailystruggle.rtp.api.server.RTPServerAccessor) Class.forName(accessorClassName).getDeclaredConstructor().newInstance();
                RTP.scheduler = (io.github.dailystruggle.rtp.api.scheduling.RTPScheduler) Class.forName(schedulerClassName).getDeclaredConstructor(JavaPlugin.class).newInstance(this);
            } catch (Exception e) {
                e.printStackTrace();
                onDisable();
                return;
