...
/**
 * Platform-neutral utility translating legacy color/format codes and hex notations
 * into canonical MiniMessage tags.
 *
 * <p>Supports:
 * <ul>
 *   <li>Single-character codes ({@code &c}, {@code §c}, uppercase & lowercase)</li>
 *   <li>Bukkit hex sequences ({@code §x§r§r§g§g§b§b} / {@code &x&r&r&g&g&b&b})</li>
 *   <li>Direct hex forms ({@code #rrggbb}, {@code &#rrggbb}, {@code §#rrggbb})</li>
 * </ul>
 *
 * <p>Preserves existing MiniMessage tag spans (e.g. {@code <gradient:#ff0000:#00ff00>})
 * verbatim so arguments inside tags are never rewritten. Stateless and thread-safe.
 */
public final class LegacyToMiniMessageNormalizer {
  ...
  // Bukkit hex form: section xsection rsection rsection gsection gsection bsection b (or with '&').
  ...
  private LegacyToMiniMessageNormalizer() {
    // Utility class.
  }

  /**
   * Translates legacy color/format codes and hex codes in {@code text} to equivalent
   * MiniMessage tags, leaving existing MiniMessage markup intact.
   *
   * @param text input text with optional legacy or hex codes; null returns null
