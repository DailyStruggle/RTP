...
| `rtp-proxy-ADR-005` | `redis-binding` | Phase 2 | Proposed | Lettuce-backed binding; Dragonfly/KeyDB parity contract |
| `rtp-proxy-ADR-006` | `velocity-bootstrap` | Phase 2 | Proposed | Velocity plugin entry, `ServerPreConnectEvent` flow, Brigadier wiring |
| `rtp-proxy-ADR-007` | `postgres-binding` | Phase 3 | Proposed | `LISTEN/NOTIFY` + `SELECT … FOR UPDATE SKIP LOCKED` binding |
| `rtp-proxy-ADR-008` | `bungee-bootstrap` | Phase 3 | Proposed | BungeeCord/Waterfall single-artifact entry, `ServerConnectEvent` flow |
| `rtp-proxy-ADR-009` | `generic-sql-binding` | Phase 4 | Proposed | Portable polling-based fallback binding (MySQL/MariaDB/H2/SQLite-dev) |
| `rtp-proxy-ADR-010` | `security-hardening` | Phase 4 | Proposed | HMAC envelope, `schemaVersion` negotiation, kill switch |

Additional ADRs may be inserted at the tail of the series (`-ADR-011`, `-012`, …) as needs arise. **Do not** renumber existing ADRs.
