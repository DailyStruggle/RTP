...
function Get-RepoRoot {
  # devstack lives at <repo>/rtp-proxy/devstack; repo root is two levels up.
...
function Invoke-GradleBuild {
  ...
  if (Test-Path $pluginLibs) {
    ...
    $proJars = $allJars | Where-Object { $_.Name -match '^RTP-Pro-' }
    if ($proJars -and $proJars.Count -gt 0) {
      $pJars = $proJars
      $variant = 'Pro'
    } else {
      $pJars = $allJars | Where-Object { $_.Name -notmatch '^RTP-Pro-' }
...
function Sync-ProxyJars {
  ...
  # RTP-Pro-<ver>.jar carries Bukkit (plugin.yml), Fabric (fabric.mod.json),
