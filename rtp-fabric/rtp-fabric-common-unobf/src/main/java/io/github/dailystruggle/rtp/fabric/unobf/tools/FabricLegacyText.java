    ...
    public static Component parseInteractive(String raw,
        ...
        try {
            ...
            if (hasHover) {
                ...
                // Reflective instantiation: in MC 26.x mojmap, HoverEvent is sealed
                // and the legacy two-arg ctor is gone (replaced by per-action records
                // like HoverEvent.ShowText). Reflection lets the unobf carrier compile
                // under both shapes and degrade at runtime if the ctor is absent.
                Object hoverEv = HoverEvent.class.getConstructor(HoverEvent.Action.class, Component.class)
                        .newInstance(HoverEvent.Action.SHOW_TEXT, hoverComp);
            ...
            if (hasClick) {
                ...
                Object clickEv = ClickEvent.class.getConstructor(ClickEvent.Action.class, String.class)
                        .newInstance(ClickEvent.Action.SUGGEST_COMMAND, suggestion);
