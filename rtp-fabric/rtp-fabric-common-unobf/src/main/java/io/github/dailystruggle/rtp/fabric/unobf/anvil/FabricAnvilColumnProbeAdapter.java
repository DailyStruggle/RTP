package io.github.dailystruggle.rtp.fabric.unobf.anvil;
...
/**
 * Fabric-side adapter exposing an {@code rtp-anvil} {@link ColumnProbe} as the
 * platform-neutral {@link ChunkColumnProbe} consumed by
 * {@code VerticalAdjustor.adjustFromProbe}, {@code PregenTask.evaluateProbe},
 * and {@code ScanTask.evaluateScanProbe} (ADR-016 / BIOME_LOOKUP_PERF_PLAN).
 *
 * <p>Companion to {@code rtp-spigot-common}'s {@code AnvilColumnProbeAdapter}.
 * Identical contract (palette identifiers reconciled to the form downstream
 * config-comparison code expects) with one platform-specific difference: Fabric
 * has no {@code org.bukkit.Material} enum to reconcile against, so the
 * reconciler is the platform-neutral {@link PaletteIdentifierNormalizer}
 * (namespace-strip + upper-case) — exactly the form
 * {@code SafetyKeys.unsafeBlocks} entries are normalised into for matching on
 * Fabric. Modded ids round-trip unchanged beyond namespace strip + case fold,
 * matching {@code FabricRTPChunk.isSafe(...)}'s comparison shape.</p>
 *
 ...
 */
public final class FabricAnvilColumnProbeAdapter implements ChunkColumnProbe {

  private final ColumnProbe probe;
  private final int chunkX;
  private final int chunkZ;

  public FabricAnvilColumnProbeAdapter(ColumnProbe probe, int chunkX, int chunkZ) {
